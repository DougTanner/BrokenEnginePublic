...
Compiled third-party source links into one shared static library used by client, server, and DataPacker — its per-config output name and flag set are owned by [Prebuilts/Platforms/VisualStudio2026/AGENTS.md](Prebuilts/Platforms/VisualStudio2026/AGENTS.md) (which also covers how to register a new unit). Most compiled libraries are wrapped by a unity-build `.cpp` in `Prebuilts/Source/Engine/` (runtime consumers) or `Prebuilts/Source/DataPacker/` (offline asset tooling) that `#include`s the upstream sources; bc7enc_rdo and zlib compile directly from their upstream trees. Exception: StackWalker is a runtime consumer whose unity wrapper lives under `Prebuilts/Source/DataPacker/`. Header-only with no compilation unit: glm, gli, PerlinNoise, RenderDoc.

PCH-backed third-party *consumption* headers go through `Common/ExternalHeaders.h`, gated by `BT_CLIENT`/`BT_ENGINE`/`BT_SERVER`/`BT_DATA_PACKER`; the PCH-less AgentTools centralize their shared consumption headers in `Tools/ToolCommon/ToolCliCommon.h`. The DataPacker-only headers (DirectXTK WAV reader, tinygltf loader, gli, cmft, SPIRV-Cross, openexr, bc7enc_rdo, stb, meshoptimizer) live in the `BT_DATA_PACKER` block — so the engine PCH never sees them while the DataPacker PCH gets them in one place (config defines / per-header warning suppressions that aren't covered by the global span travel with their include). Outside those aggregation surfaces, third-party headers appear only inside library implementation units under `Prebuilts/Source/`.
