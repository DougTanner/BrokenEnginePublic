...
- **`BT_ENGINE` guards C++-only syntax**: defined only during C++ compilation, not GLSL. C++-only constructs (`#pragma once`, `#include`, `constexpr`) sit inside `#if defined(BT_ENGINE)`; GLSL-only syntax goes in `#else`. Anything added to this wrapper must observe the same split.

## See Also

- [Engine/Data/Shaders/CLAUDE.md](../../../../Engine/Data/Shaders/CLAUDE.md) - Engine shader system, shared GLSL includes, and all shader-family sources
