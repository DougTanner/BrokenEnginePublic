...
Game-owned command handlers expose simulation, scene, UI, capture, save/replay, and diagnostic operations through the engine [agent transport](../../../../Engine/Source/Agent/AGENTS.md). Shared commands dispatch first, then side-specific handlers under `BT_CLIENT` or `BT_SERVER`.
...
- Client handlers own capture, window state, UI/input automation, and scene queries. Capture paths deliberately accept full paths where the command contract requires them.
- Server handlers own simulation controls, status-change injection, frame/profile queries, and save/replay operations. Save/load `file` values are appdata-relative bare filenames: reject empties, embedded NUL, separators, `..`, `:`, and Windows reserved device basenames.
