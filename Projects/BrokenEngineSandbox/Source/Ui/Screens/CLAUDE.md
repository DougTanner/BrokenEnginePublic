...
ImGui screens rendered by engine's `ImGuiManager`. The `.cpp` bodies are fully `#if defined(BT_CLIENT)`-wrapped and client-vcxproj-only, except `DeathMenuScreen`, which is unguarded and compiles in both vcxprojs. Headers stay unguarded so they parse in any TU (`HudScreen.h` guards its members; `TweaksScreen.h` is fully guarded — its base exists only on the client).
