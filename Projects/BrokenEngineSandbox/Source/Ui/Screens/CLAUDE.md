...
- **TweaksScreen** (`TweaksScreen/`) - Game override of `engine::TweaksScreenBase`. Implements `RenderHexShieldSection()` and `RenderWindDepositsSection()` (pure virtual in base) and `RenderLightingEffectsTab()` (virtual in base) for the Effects tab in the Lighting section. The constructor inserts game-specific `game::Wrapper` globals (including ~50 lighting wrappers from `LightingWrappers.h`) into `engine::TweaksSliderMap::Get()`. `Render()` is guarded by `mbShowImGui`. Split into `TweaksScreen.cpp`, `TweaksScreenHexShield.cpp`, `TweaksScreenWindDeposits.cpp`, and `TweaksScreenLightingEffects.cpp`
...
- **GraphicsMenuScreen** - Rendering settings panel exposing engine Wrapper variables. Time of day slider appears only in main menu. Includes the `gOpaqueUi` toggle, a `gUiOpacity` slider (disabled when opaque is on), and calls `RegisterOpaqueRect()` after rendering its window
