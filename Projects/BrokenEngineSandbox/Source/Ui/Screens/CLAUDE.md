...
- **HudScreen** - In-game overlay showing player shield and armor bars with icons, and a weapon mode toggle (Blasters/Missiles) rendered as an ImGui window with a toggle button. The button sends a weapon mode request via `Client`. Has Initialize/Shutdown lifecycle for Vulkan descriptor management
