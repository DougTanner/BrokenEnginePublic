...
- **Wrapper grouping**: Game wrappers are split across `Wrapper`, `LightingWrappers`, `ParticleWrappers`, `SmokeWrappers` pairs. Each pair's declaration order must mirror its matching `Screens/TweaksScreen/TweaksScreen*.cpp` tab. `ParticleWrappers` is in **both** vcxprojs (per-explosion-type pointers are referenced unconditionally at type-registration sites in `Missiles.cpp`/`Players.cpp`/`Spaceships.cpp`) — unlike `LightingWrappers`/`SmokeWrappers`, which are client-only by file inclusion.
