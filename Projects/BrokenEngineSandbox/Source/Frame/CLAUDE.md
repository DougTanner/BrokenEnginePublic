...
Player spaceship controller with phase-separated state. Handles input processing, weapon firing with cooldowns, and collision response. Terrain collision pushes player away from elevated terrain with velocity capped to prevent extreme acceleration. Entity collision implements shield/armor damage system with shield regeneration after cooldown. Owns a hex shield that visualizes damage direction with intensity decay. Render phase uses BufferManager's skinning allocator (`AllocateMeshData`/`AllocateJointMatrices`) to obtain contiguous regions in shared GPU buffers, then calls `AnimationData::EvaluateAnimation()` to evaluate skeletal animation and compute mesh world matrices, normal matrices, and joint matrices for GPU upload. Owns a `wind_trail_t` ID and calls `WindTrailsInterpolate::Sync()` with player-specific intensity, width, and length multiplier tweakables to contribute to wind simulation input.

**Weapon Systems**:
- Blasters fire from alternating barrels at 50ms intervals with angle jitter and interpolated spawn positions accounting for player velocity. Player blaster type sets `fWindIntensity = 1.0f` to produce wind trails. Spawn passes player-specific trail wrapper values (width, intensity, length multiplier) to each blaster instance
