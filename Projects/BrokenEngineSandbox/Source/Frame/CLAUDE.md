...
- **StatusChange.h** - Status change types and `TransferData` struct for cross-cell entity migration. `TransferData` uses deducing-this `SharedMembers()` for shared CRC subset and carries a `global_player_t`, client GUID (two 64-bit halves, not serialized over the network), and navigation delay to preserve full player state across cell boundaries. `UpdatePlayerData` carries a player UUID, weapon mode flag, and navigation delay — used by `kUpdatePlayer` to apply per-player behavior settings injected by the server session
