...
Game frame state extending engine `FrameBase` with collections for players, blasters, missiles, spaceships, and targets. Fixed timestep from `engine::kiTickRate`, phase-separated (FrameInterpolate, FramePostRender) for deterministic replay and shared-CRC validation. World-space cells are 600x600 engine units; each cell (including origin) holds 1-4 deterministically-generated island placements (world position, rotation, island CRC) emitted by `engine::GenerateIslandPlacements` via rotated-AABB rejection sampling against per-template anisotropic `mfQuadFootprintX` / `mfQuadFootprintY` from `engine::IslandTerrain` (DataPacker auto-crops each island to its land bbox at bake time). The island CRC for each placement is drawn from `engine::gpIslandTerrain->mIslandCrcsSorted`, so adding a new island folder (`Engine/Data/Islands/03/`, `04/`, …) automatically widens the variety with no code changes. Placements live in each `FrameStaticData::islands` list, so client and server agree on island layout without broadcast. `Frame::kiVersion` aggregates `engine::kiNavDataVersion` plus every collection's `kiVersion`; bumping any sub-version invalidates persisted frames.
