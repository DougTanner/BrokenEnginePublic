...
- **Owner-set destination flag**: The owning Spaceship sets the entry's `kDestination` flag after its arrival grace period (documented at the [Collections hub](../CLAUDE.md)). Missiles validate a tracked target each tick by confirming its row still exists and still carries `kDestination`, releasing otherwise.
