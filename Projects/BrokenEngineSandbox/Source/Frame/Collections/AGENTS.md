...
See also: [Engine collections hub](../../../../../Engine/Source/Frame/Collections/AGENTS.md) for the Collection\<T\> framework, Sync/Controller patterns, CRC architecture, and shared conventions — not duplicated here.

## Key Classes/Systems

- **[Blasters](Blasters/AGENTS.md)** - Fast-moving energy projectiles with terrain impact effects and swept sphere collision
- **[Missiles](Missiles/AGENTS.md)** - Guided homing missiles with AI tracking, area damage, and smoke trails
- **[Targets](Targets/AGENTS.md)** - Trackable world positions for missile guidance, referenced by stable ID with subscriber-based lifetime
- **[Spaceships](Spaceships/AGENTS.md)** - AI-controlled enemies with health, weapons, skeletal animation, and terrain interaction
- **[Players](Players/AGENTS.md)** - Player spaceships with stable ID lookup, AI behavior, weapons, shields, and cross-cell transfer

## Architecture Notes

Conventions below are hub-owned; child AGENTS.mds document only deviations.
...
**Cross-Cell Transfer**: All collections use a two-phase approach: PostCollision resolves accepted collision or terrain events before flagging an out-of-bounds survivor with `kTransfer`, then a dedicated Transfer phase generates `TransferRequest`s and destroys the entity. PreCollision supplies an exclusive frame-exit cutoff so entity hits at the boundary or later are suppressed. This separation ensures AreaDamage can skip transferring entities. Spawn and Transfer sites validate position/velocity XMVECTORs via `common::ValidateVector<IS_POSITION>()` to catch W-lane corruption at grid-boundary handoff.

**PreCollision Collision Layers**: Players, Blasters, Missiles, and Spaceships each register a collision layer in `PreCollision`, building per-entity flags, geometry, explicit movement intervals, and optional cutoffs into heap-suppressed `thread_local` vectors (the per-Frame tick fans out via `Dispatch`). Those arrays' `.data()` must stay valid through `PostCollision`, so the workbuffer cannot back them.
