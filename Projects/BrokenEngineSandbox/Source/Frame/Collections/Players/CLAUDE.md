...
- **Nav mode sentinel** (packed `value + 1` into `PlayerFlags` bits 8-10): `-1` roam via `ComputeAiSteering`, `0`-`3` cardinal frame-change, `4` island destination, `5` flagship follow. Mode-4 and mode-5 RNG consumption is held identical so mode flips (mode 5 → 4 zeroes the destination at the same tick, triggering mode 4's entry-block draws) do not desync the shared random engine: mode 5 unconditionally draws `Random(rTemplate.mfQuadFootprintX, …)` then `Random(rTemplate.mfQuadFootprintY, …)` per tick; mode 4 draws the same two (in the same order) only on the entry tick when `pVecIslandDestinations[i].W == 0.0f` (steady-state mode 4 draws zero — that's fine, both client and server see the same flags+W). Placement selection in both modes is `rStaticData.islands.at(playerIndex % rStaticData.islands.size())` so the chosen template's `mfQuadFootprintX`/`mfQuadFootprintY` matches.
