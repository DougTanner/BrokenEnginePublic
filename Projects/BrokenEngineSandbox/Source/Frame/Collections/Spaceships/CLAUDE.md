...
AI-controlled enemy spaceships with health, weapons, freeze time, and per-instance skeletal animation. `Register()` registers four types, each idempotent via a `0xFF` sentinel guard: spaceship explosion, missile-homing target, enemy blaster (with camera-aligned point light on client), and a client-only hit-flash point-light controller.
