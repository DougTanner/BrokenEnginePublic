...
**Wind Deposit**: Each blaster owns a `wind_deposit_t` ID and per-instance SOA fields for deposit intensity and area (`pfWindDepositIntensities`, `pfWindDepositAreas`), set at spawn time from `SpawnInfo`. Calls `WindDepositsInterpolate::Sync()` using these per-blaster values to update position, intensity, area, and length multiplier. Wind deposits are filtered by `BlastersType::fWindIntensity` - only blaster types with non-zero intensity produce wind deposits, allowing per-type control (e.g., player blasters deposit wind while enemy blasters do not). Player blasters and spaceship blasters use separate wrapper globals for independent tuning of deposit area and intensity.
...
**Phase Separation**: `MissilesInterpolate` holds rendering state (positions, directions, owned object IDs including wind deposit, destroyed times). `MissilesPostRender` holds logic state (velocities, targets, AI parameters, acceleration, stored directions, explosion directions). Missiles use the global default wind deposit wrappers (area, intensity, length multiplier).
...
**Owned Objects**: Each spaceship owns a pusher (air displacement), target (for missile tracking), wind deposit, and optionally a smoke trail (behind `SPACESHIP_SMOKE_TRAILS` define). Target type registered at static initialization via `TargetsPostRender::RegisterType()`. Targets created via `TargetsPostRender::Add()` in Spawn, synced via `TargetsInterpolate::Sync()` in Interpolate::Update, and removed via `TargetsPostRender::Remove()` when exploding starts. Pushers and wind deposits synced via helper function `SyncSpaceship()` using `WindDepositsInterpolate::Sync()` with spaceships-specific deposit wrappers (area, intensity, length multiplier). When `SPACESHIP_SMOKE_TRAILS` is defined, trail type is registered in `Register()`, trails are created in `Spawn()`, synced in `Update()`, and removed in `Destroy()`. Wind deposit rendering is disabled when smoke trails are active.
