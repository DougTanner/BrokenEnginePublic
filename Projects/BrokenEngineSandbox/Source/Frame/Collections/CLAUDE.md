...
- **Sync-written fields**: Written by owners via `Sync()` every frame. Also copied via `std::memcpy()` in AllocateAndCopy() so render interpolates always have valid data before Sync runs.
