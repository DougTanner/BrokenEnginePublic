...
**File Splitting**: When a collection's `.cpp` exceeds size guidelines, split by **subsystem/domain** across multiple `.cpp` files sharing a single `.h`, with file names describing responsibility (navigation, combat, update) rather than lifecycle phase. `{Name}.cpp` is the default bucket for registration, lifecycle, and phase orchestrators; `{Name}Render.cpp` (`#ifdef BT_CLIENT`) holds rendering. Players and Spaceships are split this way.
...
**Pending Tick Countdown**: Behavior-changing StatusChanges activate via `uint8_t` countdown fields rather than absolute tick comparison — see "Delayed StatusChange activation" in [../CLAUDE.md](../CLAUDE.md). Does NOT apply to entity transfers, spawns, or destroys.
