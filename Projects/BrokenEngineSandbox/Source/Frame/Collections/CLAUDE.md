...
**Wind Deposit**: Each blaster owns a `wind_deposit_t` ID and calls `WindDepositsInterpolate::Sync()` to update position and intensity. Wind deposits are filtered by `BlastersType::fWindIntensity` - only blaster types with non-zero intensity produce wind deposits, allowing per-type control (e.g., player blasters deposit wind while enemy blasters do not).
...
**Phase Separation**: `MissilesInterpolate` holds rendering state (positions, directions, owned object IDs including wind deposit, destroyed times). `MissilesPostRender` holds logic state (velocities, targets, AI parameters, acceleration, stored directions, explosion directions).
...
**Owned Objects**: Each spaceship owns a pusher (air displacement), target (for missile tracking), wind deposit, and optionally a smoke trail (behind `SPACESHIP_SMOKE_TRAILS` define). Target type registered at static initialization via `TargetsPostRender::RegisterType()`. Targets created via `TargetsPostRender::Add()` in Spawn, synced via `TargetsInterpolate::Sync()` in Interpolate::Update, and removed via `TargetsPostRender::Remove()` when exploding starts. Pushers and wind deposits synced via helper function `SyncSpaceship()` using `WindDepositsInterpolate::Sync()`. When `SPACESHIP_SMOKE_TRAILS` is defined, trail type is registered in `Register()`, trails are created in `Spawn()`, synced in `Update()`, and removed in `Destroy()`. Wind deposit rendering is disabled when smoke trails are active.
