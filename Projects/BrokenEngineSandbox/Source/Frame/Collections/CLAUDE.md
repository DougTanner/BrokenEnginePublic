...
**Wind Trail**: Each blaster owns a `wind_trail_t` ID and per-instance SOA fields for trail intensity, width, and length multiplier (`pfWindTrailIntensities`, `pfWindTrailWidths`, `pfWindTrailLengthMultipliers`), set at spawn time from `SpawnInfo`. Calls `WindTrailsInterpolate::Sync()` using these per-blaster values to update position, intensity, width, and length multiplier. Wind trails are filtered by `BlastersType::fWindIntensity` - only blaster types with non-zero intensity produce wind trails, allowing per-type control (e.g., player blasters deposit wind while enemy blasters do not). Player blasters and spaceship blasters use separate wrapper globals for independent tuning of trail width, intensity, and length multiplier.
...
**Owned Objects**: Each spaceship owns a pusher (air displacement), target (for missile tracking), wind trail, and optionally a smoke trail (behind `SPACESHIP_SMOKE_TRAILS` define). Target type registered at static initialization via `TargetsPostRender::RegisterType()`. Targets created via `TargetsPostRender::Add()` in Spawn, synced via `TargetsInterpolate::Sync()` in Interpolate::Update, and removed via `TargetsPostRender::Remove()` when exploding starts. Pushers and wind trails synced via helper function `SyncSpaceship()` using `WindTrailsInterpolate::Sync()` with spaceships-specific trail wrappers (width, intensity, length multiplier). When `SPACESHIP_SMOKE_TRAILS` is defined, trail type is registered in `Register()`, trails are created in `Spawn()`, synced in `Update()`, and removed in `Destroy()`. Wind trail rendering is disabled when smoke trails are active.
