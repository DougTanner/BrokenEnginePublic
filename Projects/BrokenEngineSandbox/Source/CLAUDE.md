...
- [Agent/](Agent/) - Agent command dispatcher (`game::ExecuteAgentCommand`, activation compile-gated by `kbAgent` — only the construction site is `if constexpr (kbAgent)`; code always compiles). Shared commands: ping / quit / get_logs / set_log_level. Unmatched commands fall through to a side-specific TU: `AgentCommandsClient.cpp` (`BT_CLIENT`) adds screenshot / dump_render_target — capture handlers fill a Graphics request mailbox and complete asynchronously via `AgentCommandServer::DeferResponse` — plus UI-automation commands (describe_ui / click / hover / set_slider / key / mouse) that drive the engine `AgentUiRegistry`/`AgentInput` harness (see [Engine Source](../../../Engine/Source/CLAUDE.md)) and complete through the same deferred-response path, plus a synchronous (non-deferred) `describe_scene` query returning camera / UI-state / units (players + spaceships with world and `WorldToScreen` screen positions, capped by `maxUnits`) / fleet / counts / islands JSON read from the committed frame snapshot; `AgentCommandsServer.cpp` (`BT_SERVER`) adds sim-control commands (status, pause, timescale, save, load, reset, replay record/play) that wrap the existing debug-control paths and StatusChange-injection commands (inject_status_changes / spawn_players — queued into the broadcaster and folded into the next tick's inputs; rejected while clients await spawn or during replay playback, spawn count capped, coords validated); the JSON frame-read queries (query_frame / query_players / query_collection over the Frame SOA collections) live in `AgentCommandsServerQueries.cpp` (`BT_SERVER`). Server save/load `file` params are a trust boundary — validated bare (no separators / `..` / `:` / empty; land appdata-relative); client capture `path` params deliberately accept arbitrary full paths. Replay commands throw where `kbDebugInput` is compiled out
