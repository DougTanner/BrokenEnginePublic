...
- **Compile-time toggles**: `Pch.h` contains `inline constexpr` flags for reconcile threading, frame dispatch parallelism, network simulation levels, smoke spread testing, auto-launching and auto-connecting to a local server, the default log level threshold (`keLogLevelDefault`) and focused log category (`keFocusedLogCategoryDefault`), and other debug/profile features, used with `if constexpr` for zero overhead
