...
- **Compile-time toggles**: `Pch.h` contains `inline constexpr` flags for reconcile threading, frame dispatch parallelism, network simulation levels, auto-launching and auto-connecting to a local server, per-category log level thresholds (one per log category), and other debug/profile features, used with `if constexpr` for zero overhead
