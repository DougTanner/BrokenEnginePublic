...
- **Compile-time toggles**: `Pch.h` contains `inline constexpr` flags for reconcile threading, frame dispatch parallelism, network simulation levels, auto-launching and auto-connecting to a local server, and other debug/profile features. It includes `LogTypes.h` first, then defines `keLogLevels[]` — a `constexpr LogLevel` array (one entry per log category) — before including `Common.h`, so that the `LOG()` macro is available in all headers including engine-level ones
