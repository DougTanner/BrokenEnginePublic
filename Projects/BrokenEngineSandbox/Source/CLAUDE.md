...
- **Game** (`Game.h`/`Game.cpp`) - Central coordinator inheriting from `engine::GameBase`, accessed via `gpGame`. Manages lifecycle (menu/gameplay/death), multi-player-per-client tracking (a list of owned `global_player_t` IDs with a focused index), `HumanPlayerIndex()` (resolves the focused player's SOA index from `FramePostRender`), `FocusNext()`/`FocusPrev()` navigation, multi-frame grid orchestration, cross-grid entity transfers, sound settings and tweaks screen state persistence, and music playlists. Owns `ClientSession` (client) or `ServerSession` (server) for networking. Holds a visual error offset used by the camera to smooth abrupt reconciliation corrections; decayed each render frame by `GameBase`, cleared on reset
...
- **Compile-time toggles**: `Pch.h` contains `inline constexpr` flags for reconcile threading, frame dispatch parallelism, network simulation levels, smoke spread testing, auto-launching and auto-connecting to a local server, the initial log category filter (`kLogEnabledCategoriesDefault`), and other debug/profile features, used with `if constexpr` for zero overhead
