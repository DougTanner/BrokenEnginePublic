...
- **Game** (`Game.h`/`Game.cpp`) - Central coordinator inheriting from `engine::GameBase`, accessed via `gpGame`. Manages lifecycle (menu/gameplay/death), human player tracking by stable ID, multi-frame grid orchestration, and music. Owns `ClientSession` (client) or `ServerSession` (server) for networking
...
- **Deterministic floating-point**: `/fp:strict` in vcxproj ensures cross-hardware determinism for CRC-based reconciliation
...
- **Compile-time toggles**: `Pch.h` contains `inline constexpr` flags for reconcile threading, frame dispatch parallelism, and network simulation levels, used with `if constexpr` for zero overhead
