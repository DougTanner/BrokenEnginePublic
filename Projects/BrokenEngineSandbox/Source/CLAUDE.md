...
- **Game** (`Game.h`/`Game.cpp`) - Central coordinator inheriting from `engine::GameBase`, accessed via `gpGame`. Manages lifecycle (menu/gameplay/death), human player tracking by stable ID, multi-frame grid orchestration, cross-grid entity transfers, sound settings persistence, and music playlists. Owns `ClientSession` (client) or `ServerSession` (server) for networking. Holds a visual error offset used by the camera to smooth abrupt reconciliation corrections; decayed each render frame by `GameBase`, cleared on reset
