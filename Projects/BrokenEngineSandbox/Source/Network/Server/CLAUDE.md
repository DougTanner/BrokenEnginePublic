...
- **Reconnect / load relink**: reconnect (`TryRelinkNewClient`) sorts GUID matches by global ID to preserve creation order before rebuilding the parallel vectors; the load path (`TryRelinkClientForLoad`) relinks in frame-map encounter order (alignment holds, creation order doesn't). On load, the client/transfer/broadcaster managers get `ResetState()` but the fleet manager only `ClearPendingRequests()` — `ReadFleetData` + `OnResetForLoad` just restored its state, and the pending flagship updates restoration re-queued must not be cleared again.
