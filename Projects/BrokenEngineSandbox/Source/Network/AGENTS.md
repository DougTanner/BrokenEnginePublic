...
Game-layer multiplayer orchestration. `ClientSession` and `ServerSession` extend their engine base classes and are conditionally compiled (`BT_CLIENT` / `BT_SERVER`); see the subdirectories. The top-level files are side-agnostic: packet identifiers (`game::`), player-event / fleet-sync parsing (`game::`), and the `StatusChange` wire codec (declared `engine::` because the engine transfer path calls it, even though the per-type payloads are game data). Engine-level networking conventions (channel math, send path, ACK model, ENet tuning) are documented in [Engine/Source/Network/AGENTS.md](../../../../Engine/Source/Network/AGENTS.md) — not restated here.
