...
- **ClientReconciler** - Rollback-and-replay reconciliation on a dedicated `PersistentWorker` thread. Encapsulates reconciliation state with controlled access from `ClientSession`. Re-syncs human identity fields from `gpGame` at `Kick()` time to ensure the worker always has fresh human state
...
- **ClientSession companion files**: `ClientSession.cpp` (core/reconcile integration), `ClientSessionSubscriptions.cpp` (coord subscription management with quadrant neighbor computation, death screen subscription handling, full-state application)
