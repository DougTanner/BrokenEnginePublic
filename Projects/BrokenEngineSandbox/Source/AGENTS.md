...
- **Save/load/replay** (`Save/GameSaveLoad`, server-only, driven by the engine server loop): grid saves are atomic and coord-key sorted. Externally reported saves consume the write result; replay start requires its grid snapshot, while replay stop attempts every component and reports success only when all persist. Each per-coord recording writer becomes terminal at its first eviction and retains that coord's last complete Frame; reactivation never resumes it. Playback readers retire independently at their recorded endpoints, and the replay loops only after the final reader retires. A terminal writer missing its retained Frame deletes its sibling files without preventing later writers or metadata from being attempted. Replay validates each tick's checksum against resimulation. Successful loads retain the saved simulation clock; direct loads resynchronize clients against that timeline. Nav/elevation data is derived, not persisted; clients receive server-built nav data. Fresh-game paths reset the fleet manager explicitly. Save/replay reads are all-or-nothing trust boundaries: corrupt or truncated input falls back to a clean fresh game.
