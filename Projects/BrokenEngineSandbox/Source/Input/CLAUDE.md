...
- **FrameInputPressedFlags** - Per-player one-shot press flags (toggle primary, toggle secondary, toggle skill) for discrete actions
- **PlayerInput** - Per-player held flags (`FrameInputHeldFlags`: primary, secondary, zoom), move vector, and direction. Used by the network protocol for server-to-client delta updates and reconciliation extrapolation, but not part of `FrameInput`
