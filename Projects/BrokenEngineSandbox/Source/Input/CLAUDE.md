...
- **`FrameInput` serialization**: layout is versioned — bump `kiVersion` on any change to the stream format or `StatusChange` payloads (replays validate version and size). `Crc()`/`operator==` serve the replay `DifferenceStream` (skip writing identical consecutive differences) and diagnostics; reconciliation desync detection compares `Frame` CRCs, not `FrameInput::Crc()`.
