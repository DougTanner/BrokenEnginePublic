...
- Scroll ownership: any open menu owns all wheel input regardless of cursor position, except the actual main-menu background (`InMainMenu()` with `UiState::kPause`) when `kbDebugInput` is enabled; a live ImGui context also owns it when it requests mouse capture. The camera receives a wheel delta only when neither owns it. Always advance the baseline for the running lifetime total when input is swallowed, so it cannot surface later as zoom applied after the fact, and inspect ImGui IO only after confirming a live context.
