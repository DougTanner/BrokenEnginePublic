...
- Scroll ownership: any open menu owns all wheel input regardless of cursor position; otherwise a live ImGui context owns it when it requests mouse capture. The camera receives a wheel delta only when neither owns it. Always advance the lifetime-accumulator baseline for swallowed input so it cannot surface later as deferred zoom, and inspect ImGui IO only after confirming a live context.
