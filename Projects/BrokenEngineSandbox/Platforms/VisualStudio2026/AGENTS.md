...
- **PreBuildEvent**: builds the ThirdParty static lib and `DataPacker.exe` only if missing — existence check, not freshness; stale artifacts need a manual rebuild. DataPacker is always built Release regardless of game config. See [ThirdParty Prebuilts AGENTS.md](../../../../ThirdParty/Prebuilts/Platforms/VisualStudio2026/AGENTS.md).
