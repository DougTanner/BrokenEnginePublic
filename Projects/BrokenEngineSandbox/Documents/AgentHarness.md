...
`replay_transfer_fixture` is accepted during active recording, or only while the server is paused with an unconsumed `replay_record {"start":true}` transition. It rejects playback, ordinary non-recording use, an unpaused pending start, cancelled starts, invalid source frames, and invalid coordinates. Its optional Boolean `pauseAfterWriterInput` pauses the server immediately after the first normal writer input; the result echoes the applied Boolean. Its event is harvested at tick `E`; the first normal writer input is `E + 1`.
...
H3. Mutate the generation digest; use invalid fullframes flag, unknown kind, origin-key coordinate entry, reordered/duplicate inventory entry, missing/surplus entry, negative count/size, and trailing data. Reject each.
H4. On Debug, every coord has a fullframes entry and byte-identical sibling; on non-Debug, static inspection proves `kbReplayFullFrames == false` and v3 has no kind-5 entry. Do not enable runtime Release diagnostics.
...
- `key`: `{"key":name,"holdFrames"?:1}`. Supports case-insensitive single letters/digits; `ESC`/`ESCAPE`, `SPACE`, `TAB`, `ENTER`/`RETURN`, `UP`/`DOWN`/`LEFT`/`RIGHT`, and `F1`–`F24`. Returns `{"ok":true}`.
