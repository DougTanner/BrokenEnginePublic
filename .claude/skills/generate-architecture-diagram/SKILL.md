...
Use the Agent tool to launch an Explore subagent (`model: "opus"`) analyzing the target path. Prompt it to report:
...
Write to `Documents/Architecture/<Name>.md` — flat, no subdirectories. `/update-architecture-diagrams` discovers diagrams by globbing `Documents/Architecture/*.md`, so a file in a subdirectory would never be updated. Default template (existing diagrams follow it):
