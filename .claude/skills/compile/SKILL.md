...
DataPacker coordinates across worktrees: it serializes through the PC-global `"BrokenEngineDataPacker"` mutex and reuses `%TEMP%\DataPacker\<Project>`. Do not add another DataPacker lock or checkout-local cache copy. Gaea raw and split intermediates use the single mutable `%TEMP%\DataPacker\<Project>\Gaea\Islands` cache; source-tree island leaves retain only tracked BC outputs.

### 2. Build the Project

**IMPORTANT: Build duration & foreground default** — Typical builds (incremental, selective, even ThirdParty rebuilds) finish in seconds to a few minutes. **Run builds in the FOREGROUND with the default 2-minute timeout** — blocking is reliable in every context (subagents especially: turn-ends and background notifications are flaky). The ONE exception that far exceeds it (upwards of an hour) is a build that triggers a widespread DataPacker data re-export — re-exporting many islands (slow Gaea export) or textures (slow texture compression); use `run_in_background: true` only when that re-export is expected from widespread source/version/shared-chunk invalidation or Gaea-cache invalidation. A missing or invalid aggregate `.pack` alone is rebuilt from clean shared chunks and does not inherently re-export their inputs. If a foreground build is killed at the cap it leaves an `unsuccessfulbuild` marker in the tlog directory (forces a full rebuild next time) — recover by re-running that build in background, not foreground.
