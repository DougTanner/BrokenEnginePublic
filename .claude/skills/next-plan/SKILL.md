...
Before root step 12 substep 1 on every completion, terminal-abandonment, or rejection route, run two independent `/session-audit` reviews on the resulting changed queue files and resolve findings through root step 10's fix/re-review rule. Queue cleanup must be reviewed before it is committed or landed.

- **After execution completes**: the user approved in Step 9 and C++ Code Change Process steps 2–11 completed with residuals routed. Remove the plan's row and file, prune every target reference from `## Dependencies` and `## File Groups`, and delete entries that then name fewer than two live plans. Include all cleanup in the reviewed session commit. Run root step 12 substeps 1–7, verify the landed primary commit and that `lock status --domain plan --key $TargetPlan` still reports `$Owner`, then run `lock release --domain plan --key $TargetPlan --owner $Owner`. Only after that release succeeds run root step 12 substep 8 to remove the session worktree and branch.
- **Terminal abandonment**: Step 3 classified the plan Obsolete, or the user chose "abandon and clean up" in Step 3/4 — remove the row/file and perform the same dependency/file-group pruning, root step 12 substeps 1–7, owner-checked plan release, and root step 12 substep 8 sequence.

If the run ends any other way, do **not** delete the row or plan file. On rejection in Step 9, remove this session's `[CLAIMED <date>] ` mirror, keep the refreshed plan file, commit those queued-state changes, and run root step 12 substeps 1–7. After verifying the landed commit and matching plan owner, release the plan claim, then run root step 12 substep 8. If landing or owner verification fails, retain the plan claim, do not clean up the session worktree/branch, and report it. On an error or user stop mid-execution, retain both claim and mirror and report the metadata.
...
- **options**: `Approve` (proceed to implementation), `Reject` (the refreshed plan and mirror removal are landed through root step 12 substeps 1–7, the owner-verified plan claim is released, then substep 8 cleans the session — the plan returns to the queue)
...
If the user picks `Approve`, follow the standard C++ Code Change Process defined in the top-level `AGENTS.md`, **starting from step 2 (implementation)** — step 1 (audit and grill) already ran in 9a. Carry the user's decisions on additional candidates into the process and proceed straight into the edit in the same turn. After step 11, execute Step 8's coordinated queue cleanup, root step 12 substeps 1–7, plan-claim release, and root step 12 substep 8 sequence.

If the user picks `Reject`, follow Step 8's rejection route: strip this session's `[CLAIMED <date>] ` mirror, keep the refined plan file and row queued, land through root step 12 substeps 1–7, release the owner-verified plan claim, then run root step 12 substep 8. If landing fails, retain the claim and session worktree/branch and report it.
...
- **User rejects in Step 9 approval**: follow Step 8's rejection route — remove this session's mirror, keep the refreshed plan file and row, land through root step 12 substeps 1–7, owner-check and release through AgentCli, then run root step 12 substep 8. Retain the claim and session worktree/branch if landing fails.
