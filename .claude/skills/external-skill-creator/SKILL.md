...
- **`external-` prefix** marks explicit-invocation skills (e.g., `external-grill-plan`, `external-design-interface`, `external-deep-analysis`). Most set `disable-model-invocation: true`, but the flag removes the skill from Claude's reach entirely (no auto-trigger, no Skill tool) — set it only when nothing, neither a documented workflow step nor Claude itself, needs programmatic invocation. Three intentional exceptions: `external-grill-plan` and `external-self-audit` (the main agent invokes them during steps 2–3 of the AGENTS.md C++ Code Change Process) and `external-design-interface` (proactively suggests itself, which requires staying in the listing). Locally-created workflow skills such as `plan-audit` and `finalize-changes` omit the prefix and the flag so the process can invoke them. User-only skills without the prefix (e.g., `gaea2-load`, `next-plan`) follow the same rule. Unprefixed proactive skills (e.g., `add-collection`, `repo-code-review`, `compile`) auto-trigger on matching contexts.
