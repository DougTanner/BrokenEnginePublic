...
1. **Identify affected directories**: If invoked as a subagent, the caller should provide the list of changed files. Otherwise, enumerate files touched by Edit/Write/NotebookEdit tool calls in this session — don't use `git status`, it can't distinguish this session's edits from pre-existing changes; if session memory is insufficient, ask the user for the file list. Update AGENTS.md only in the immediate directories containing those files (not parent directories unless their content directly changed).
