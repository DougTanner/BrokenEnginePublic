...
Main.cpp orchestrates a multi-phase pipeline: a one-shot legacy-raw → zlib migration of `.BC[457]_UNORM_BLOCK` / `.R16_UNORM` intermediates (idempotent: skips files whose payload size doesn't match raw-size math), then pre-export (Scene, a Gaea bake of island intermediates, Islands) generates intermediates, offline IBL cubemap convolution runs next, main export processes asset types in parallel, then header generation and ThirdParty attribution collection. The Gaea bake is a separate step (not an `ExportJob`) that shells out to `Gaea.Swarm.exe` exactly once per island at the requested texture resolution — all Gaea-version-specific logic is confined to that one translation unit for easier migration. The elevation pipeline is meters-absolute end-to-end: Gaea emits float-raw normalized elevation, DataPacker scales by Island.json's `elevationMeters` (also patched into the archetype's `Terrain.Height` before the bake) and subtracts a per-island beach offset (`ShoreHeight × elevationMeters`, where `ShoreHeight` is read from the archetype's Sea node) so pixel value 0 means sea level (beach), and writes the heightmap as R32_SFLOAT meters. Sea floor depth is consequently per-island rather than a global constant. Elevation is then box-filter downsampled in-process to a fraction of the color resolution before being rewritten in place (elevation is sampled by a vertex grid in `Terrain.vert` and doesn't need per-texel resolution; this is the primary disk-size lever for islands). The runtime samples directly with no per-island height or water-depth multipliers. Bakes share archetype `.terrain` files across islands, so the patch+bake+restore is always wrapped under a process-local mutex and is RAII-rolled-back to byte-and-mtime-identical state on completion (no `git diff`, no sibling-island re-bakes); the patch covers archetype dimensions and every per-node `"Seed"` field (Gaea's `--seed` CLI flag only mixes a global seed into per-node randomness, so per-node fields must be overwritten directly). Single-instance guard is a named Win32 mutex. Outside the debugger, top-level `try/catch` shows a MessageBox; under the debugger it runs unwrapped so exceptions break in.
