...
void ExportGltf::Export()
{
	...
	for (const tinygltf::Material& rMaterial : gltfModel.materials)
	{
		...
		if (rMaterial.values.find("baseColorFactor") != rMaterial.values.end())
		{
			...
			gltfShaderData.baseColorFactor = XMFLOAT4(static_cast<float>(pfData[0]), static_cast<float>(pfData[1]), static_cast<float>(pfData[2]), static_cast<float>(pfData[3]));
		}

		gltfShaderData.emissiveFactor = XMFLOAT4(static_cast<float>(rMaterial.emissiveFactor[0]), static_cast<float>(rMaterial.emissiveFactor[1]), static_cast<float>(rMaterial.emissiveFactor[2]), 1.0f);

		ASSERT(rMaterial.extensions.find("KHR_materials_pbrSpecularGlossiness") == rMaterial.extensions.end());
		gltfShaderData.workflow = 0; // PBR_WORKFLOW_METALLIC_ROUGHNESS
		...
		if (rMaterial.values.find("baseColorTexture") != rMaterial.values.end())
		{
			...
			gltfShaderData.colorTextureSet = rMaterial.values.at("baseColorTexture").TextureTexCoord();
		}
		...
		if (rMaterial.values.find("metallicRoughnessTexture") != rMaterial.values.end())
		{
			...
			gltfShaderData.physicalDescriptorTextureSet = rMaterial.values.at("metallicRoughnessTexture").TextureTexCoord();
		}
		...
		if (rMaterial.additionalValues.find("normalTexture") != rMaterial.additionalValues.end())
		{
			...
			gltfShaderData.normalTextureSet = rMaterial.additionalValues.at("normalTexture").TextureTexCoord();
		}		
		...
		if (rMaterial.additionalValues.find("occlusionTexture") != rMaterial.additionalValues.end())
		{
			...
			gltfShaderData.occlusionTextureSet = rMaterial.additionalValues.at("occlusionTexture").TextureTexCoord();
		}
		...
		if (rMaterial.additionalValues.find("emissiveTexture") != rMaterial.additionalValues.end())
		{
			...
			gltfShaderData.emissiveTextureSet = rMaterial.additionalValues.at("emissiveTexture").TextureTexCoord();
		}

		if (rMaterial.values.find("metallicFactor") != rMaterial.values.end())
		{
			gltfShaderData.metallicFactor = static_cast<float>(rMaterial.values.at("metallicFactor").Factor());
		}

		if (rMaterial.values.find("roughnessFactor") != rMaterial.values.end())
		{
			gltfShaderData.roughnessFactor = static_cast<float>(rMaterial.values.at("roughnessFactor").Factor());
		}
		...
		gltfShaderData.alphaMask = 0; // ALPHAMODE_OPAQUE
		gltfShaderData.alphaMaskCutoff = static_cast<float>(rMaterial.alphaCutoff);
