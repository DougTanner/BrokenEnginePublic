...
- **Islands**: each island lives in its own folder. The Gaea pre-pass (see `../CLAUDE.md`) reads `Island.json` (optional `"mips": [...]` array; otherwise a default power-of-two chain) and the resolved `.terrain` archetype, then invokes Gaea once per mip resolution, writing `FloatRaw32` `.raw` elevation, `UshortRaw16` `.raw` AO, and `.exr` color/normals into `mip<N>/` subfolders. Any Island.json keys outside `archetype`/`seed`/`mips`/`elevationMeters` are forwarded to Gaea via `--vars` so a single template can produce islands at multiple physical scales. `widthMeters`/`elevationMeters` are a special case: Gaea's `--vars` only injects into named graph variable nodes and can't reach the archetype's intrinsic `Terrain.{Width,Height}`, so when either key is present the pre-pass patches those properties directly in the `.terrain` JSON around the bake and restores the original bytes + mtime afterward (see archetype-mutex note in `../CLAUDE.md`). Elevation is dropped from `mip<N>/` for mips above `mips[0] / kiElevationDivisor` (= 4× lower texel dim than color) — sampled by a vertex grid in `Terrain.vert`, so it doesn't need color-level resolution; this is the primary disk-size lever for islands. Gaea itself still emits elevation at every mip (the archetype graph could gate the Export node on a resolution variable for CPU savings); DataPacker deletes the unused files post-bake. The Islands `ExportJob` ingests the `mip<N>/` chain: color/normals become multi-mip BCn picked up by `ExportTexture` as passthrough, AO stays `R16_UNORM`, and elevation is converted to absolute-meters `R32_SFLOAT` (Gaea's normalized float scaled by `Terrain.Height` then offset by `-common::kfOceanDepthMeters`, so pixel 0 = sea level / beach, negative = underwater, positive = land). The CPU heightmap is the same float-meters data downsampled from `mips[iElevationStart]`. World footprint comes from `GetIslandFootprint()` (Island.json's `widthMeters` or the `.terrain`'s intrinsic `Terrain.Width`, isotropic — single value, no separate Y) and is stamped into `IslandHeader`. `EnumerateMipDirs` uses `AmbientOcclusion.r16` as the per-mip sentinel since elevation is absent at lower mips. `Handles()` ignores island folders until `mip0/AmbientOcclusion.r16` exists, so a folder containing only `Island.json` is a no-op until the bake produces one.
