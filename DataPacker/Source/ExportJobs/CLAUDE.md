...
- **Islands**: each island lives in its own folder. The Gaea pre-pass (see `../CLAUDE.md`) reads `island.json` (optional `"mips": [...]` array; otherwise a default power-of-two chain) and the resolved `.terrain` archetype, then invokes Gaea once per mip resolution, writing `UshortRaw16` `.raw` elevation/AO and `.exr` color/normals into `mip<N>/` subfolders. After all mips succeed it writes a `world.json` sidecar carrying the `.terrain`'s `widthMeters`/`heightMeters`. The Islands `ExportJob` ingests the `mip<N>/` chain into multi-mip BCn / `R16_UNORM` intermediates picked up by `ExportTexture` as passthrough, plus a mode-of-nonzero beach elevation and a float-downsampled CPU heightmap; world dimensions are stamped into `IslandHeader`. `Handles()` ignores island folders until the elevation intermediate at the canonical mip exists, so a folder containing only `island.json` is a no-op until the bake produces one.
