...
Verified chain (all citations re-grounded against the current tree):

1. `FleetNavigationController::ShiftFlagshipAfterDeath` (`FleetNavigationController.cpp:195-216`; the assignments at `:212-214`) sets `rFleet.iFlagshipIndex = iNewFlagship`, then `rFleet.wantedCoord = rFleet.members.at(iNewFlagship).coord` — the fleet's wanted coord becomes **the promoted ship's own coord** — and resets `rFleet.fFrameChangeTimer = rFleet.fNavigationDelay`.
2. `ProcessFlagshipUpdates` drains that into a `kUpdateFleet`; the Spawn-phase handler in `Players.cpp` (`:286-310`) sets `kIsFlagship` on the promoted ship (`:296`) and writes that same coord into `pFleetWantedCoords` (`:306`).
...
5. Nothing else in `ComputeNavigation` leaves mode 5: the frame-change-timer path (`PlayersNavigation.cpp:250`) is mode `-1` only, and the tail `else if (riNavDirection >= 0)` (`:398`) handles modes 0-3.
...
The follower case is **not** this defect and is already resolved: a plain wingman recovers in one tick once the reassignment lands, as recorded in the mode-5 comment (`PlayersNavigation.cpp:285-296`) and `Frame/Collections/Players/AGENTS.md`. That comment names this promoted-ship case as the known, unfixed exception; this plan is its acceptance gap.
...
- **Option A — frame-local exit (recommended for behavior; costs a version bump).** In `ComputeNavigation`, add the missing promoted-flagship case: when the entity carries `kIsFlagship`, is in mode 5, and `fleetWantedCoord == rStaticData.coord`, flip to island-seek (mode 4) and set `rVecIslandDestination = XMVectorZero()`. Recovery is one tick and needs no server round trip. **RNG draw parity is preserved exactly**: mode 5 draws 3 per tick (`PlayersNavigation.cpp:301,304,305`) and mode 4's entry block draws 3 (`:342,353,354`) gated on `XMVectorGetW(rVecIslandDestination) == 0.0f` (`:334`), so zeroing the destination on the flip tick keeps the count at 3. This changes CRC'd nav flags — see Risk tier and invariants.
...
The regions listed here are both target and ceiling: make the smallest complete change inside them and add no abstractions, configuration, refactors, or fixes to adjacent code encountered along the way. Naming a file grants no permission to touch anything in it beyond the named regions plus the mechanical necessities (includes, declarations) the named change requires.

**In scope — Option A only:**

- `PlayersNavigation.cpp` — `PlayersPostRender::ComputeNavigation`, the flagship-proximity block (`:214-247`) and/or a new promoted-flagship case adjacent to it, per the Design; and the stale portion of the mode-5 comment (`:285-296`) that names the promoted ship as the unfixed exception.
- `Frame.cpp` — the `120 +` base of the `Frame::kiVersion` definition (`:36`): a single increment, nothing else in the file.

**In scope — Option B only:**

- `FleetNavigationController.cpp` — `ShiftFlagshipAfterDeath` (`:195-216`), specifically the `fFrameChangeTimer` re-arm at `:214`; no change to the round-robin promotion scan (`:197-210`) or the pending-update push (`:215`).
- `PlayersNavigation.cpp` — only the stale portion of the mode-5 comment (`:285-296`); no code change in this file.

**In scope — either option:**

- `Frame/Collections/Players/AGENTS.md` — the Navigation Invariants bullet naming the promoted ship as the unfixed exception; update to match whichever option lands.
...
- Mode 4/5 steering behavior itself (`NavQueryDirection` contour following, the pathfind throttle and terrain-lookahead recompute at `PlayersNavigation.cpp:259-281`) — unchanged.
...
- Changing `fNavigationDelay`'s default (`Fleet.h:39`) or making it per-member.
- `TickFleetTimers`, `ProcessFlagshipUpdates`, the `kUpdateFleet` handler in `Players.cpp`, `Fleet.h`, and `Players.h` — read-only context under both options.

## Critical files

- `Projects/BrokenEngineSandbox/Source/Frame/Collections/Players/PlayersNavigation.cpp` — `PlayersPostRender::ComputeNavigation`: fleet-override block (`:172-212`), flagship-proximity block (`:214-247`), mode-5 branch and its 3-draw mirror (`:283-330`), mode-4 entry draws (`:331-361`). Option A edits here; Option B touches only the stale comment.
- `Projects/BrokenEngineSandbox/Source/Network/Server/FleetNavigationController.cpp` — `ShiftFlagshipAfterDeath` (`:195-216`, the `wantedCoord`/timer assignments), `TickFleetTimers` (`:32-113`, the 60 s recovery path and its `:96` nav-direction gate). Option B edits this file.
...
- **Tier 3.** The change sits on the determinism/CRC and save/replay-compatibility surface (a Tier-2 exclusion), and the Option A/B decision itself is staged for `/external-grill-plan`. Classify at Tier 3 regardless of which option is chosen; Option B's server-only shape does not lower the classification of this plan's review path.
- **Invariant exposure.** Inside the `/fp:strict` CRC'd tick. Option A changes CRC'd nav-flag transitions → replay-visible: `pFlags` is in `PlayersPostRender::SharedCrcMembers()` (`Players.h:295-305`), so per the rule at `Frame.cpp:33-35` the `120 +` base of `Frame::kiVersion` must be bumped, and client and server land together. No wire, `.pack`, layout, or threading exposure under either option. Option B is server-only (`BT_SERVER`) fleet orchestration with no frame-code change and therefore no version bump.
...
Live client/server harness scenario, not inspection alone: fleet with a flagship and at least two wingmen, scripted flagship kill, then nav-mode and CRC readback (plus the option-specific replay check above). Mechanics: `/agent-harness`.

## Coordination

- **Frame version/save/replay batch — applies only if Option A is chosen.** A frame-local mode transition shifts computed frame CRCs and requires bumping the `120 +` base of `Frame::kiVersion` (`Frame.cpp:36`), invalidating existing saves and replays. Co-land behind one consolidated `Frame::kiVersion` change with `Documents/Plans/Frame/PlayerTransferUuidPreservation.md`. The cell-transfer sentinel plan that previously anchored this batch has landed and consumed its own collection `kiVersion` bumps, so each remaining member owns its own increment of the `Frame.cpp:36` base rather than deferring to a last lander. Option B carries no bump and leaves that batch untouched — record the chosen option in this section at grill time so the batch members are not left expecting a co-land that will not happen.
- The cell-transfer sentinel work that shared `PlayersNavigation.cpp`'s player `TransferRequest` build region has landed; nothing is left to co-schedule there, and this plan's code citations were re-resolved against the landed tree (2026-07-24).
