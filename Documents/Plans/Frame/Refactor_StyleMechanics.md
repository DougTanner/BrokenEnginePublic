...
- Bare `memset` → `std::memset` at lines 395, 417 (rule 41; `Alignments.cpp:52` shows the house form) [~2m]
...
- `static auto AlignmentKeyLess` → `static constexpr auto` at line 8 [~2m]
...
- Member initializers on `PendingCollisionResult`/`ZoneRange` (folded into `Architecture_CollisionHeaderSurface.md` — landed and removed; the types moved to `Collision.cpp` with initializers in)
- Anything behavioral
...
- Every item re-verified against source and `Documents/C++StyleGuide.txt`:
  ...
  - Collision.cpp: raw `sTestedBGeneration[j]` at `:437/:441` vs `.at()` at `:435` — rule 16 explicitly extends to vectors ("Access std::vectors with .at()", StyleGuide:95). Bare `memset` at `:395/:417` (rule 41; `Alignments.cpp:52` `std::memcpy` precedent confirmed). Positional `{-1, 0}` at `:375` (rule 42).
  ...
  - `Alignments.cpp:8` `static auto` lambda confirmed; `constexpr` is valid for the captureless comparator.
