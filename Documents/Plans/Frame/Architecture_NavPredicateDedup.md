...
- `NavBuildSplit.md` re-read: it promotes `SegmentsIntersect` to external `engine::` linkage declared in `NavBuildInternal.h` — extending that header to `NavQuery.cpp` (third TU) or hoisting to `NavBuild.h` both remain viable; ordering note stands (the split has not landed; `NavBuild.cpp` is still one 935-line TU).
