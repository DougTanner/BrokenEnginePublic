...
- `MultiElementCrc` (`Collection.h:609-632`) and `CollectionCrc` (`Collection.h:600-606`) continue the XOR pattern at their combine sites.
...
- `MultiElementCrc` (lines 609-632) and `CollectionCrc` (lines 600-606): same conversion at the combine sites [~10m]
...
- The two-target helper `common::Crc(value, rCrc, rSharedCrc)` (`Crc.h:123-130`) fed by leaf `Crcs()` methods — Common-layer change; fold into this plan at execution only if the grill agrees the same session should take it
...
- All five XOR sites verified at the cited lines: `Collection::Crc` (:577-587), `OptionalIdToIndex::Crc` (:501-523, keys sorted into workbuffer at :507-514 so order-independence is genuinely unneeded), `MultiCrc` (:186-213), `MultiElementCrc` (:609-632), `CollectionCrc` (:600-606).
...
- `common::kCrcMultiplier` confirmed (`Crc.h:50`); the two-target helper carved out in Out-of-scope confirmed at `Crc.h:124-130` (plan's :123-130 cite includes its comment line — fine).
