...
- `BuildCellNavData` (lines 547–685, 139 lines): extract the 5-deep `if constexpr (kbDebugNavCrossingCheck)` diagnostic block (lines 634–682, compiled out in all three Pch.h configs) into a file-local `DebugCheckCrossingEdges(const NavData&)` — it already needs the cross-domain `SegmentsIntersect` that `NavBuildSplit.md` promotes [~15m]
...
- `BuildNavContour` (121 lines), `IslandTerrain::IslandTerrain()` (125 lines), `IslandChainPlacement::Generate` (117 lines) — reviewed and deliberately left intact (linear, well-sectioned; KISS)
...
- All four extraction seams verified against source:
  ...
  - `BuildCellNavData` `NavBuild.cpp:547-685` (139 lines); the `kbDebugNavCrossingCheck` block is `:634-682`, the constant is `false` in all three configs (`Pch.h:34/51/68`), and the block calls `SegmentsIntersect` (`:673`) — the cross-domain dependency `NavBuildSplit.md` promotes, as stated.
