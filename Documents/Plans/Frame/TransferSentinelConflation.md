...
- **Missile homing ramp re-rolled every crossing.** The `pfDeltaRotationDelays` member of `MissilesPostRender` decrements unconditionally every tick (`MissilesUpdate.cpp:178,192`), so it is virtually always negative by the time a missile crosses a cell edge; `Missiles.cpp:461-463` reads `rInfo.fDeltaRotationDelay > 0.0f` as "provided" and otherwise draws a fresh random — every crossing re-randomizes the delay and re-enters the soft-launch ramp (`fDelayPercent` scaling, `MissilesUpdate.cpp:185,198`), making homing missiles go briefly sluggish at each boundary. Contradicts the Missiles AGENTS.md claim that the boost-ramp delay is preserved. Same conflation on `fExhaustDelay` (`Missiles.cpp:465-467`).
