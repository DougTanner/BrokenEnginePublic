...
`Engine/Source/Frame/NavBuild.cpp` is 910 lines and approaching the 1000-line `/reduce-file` soft threshold. A codebase sweep flagged it as the strongest un-tracked split candidate. The file holds two cleanly-separable domains that share only a small set of types and one geometry predicate:
