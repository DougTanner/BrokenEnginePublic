...
|---|------|------|--------|--------|-------|-------|-------|
| 1 | [Graphics/ExtendModelPipelineRecreateToShadowAndSmoke.md](Graphics/ExtendModelPipelineRecreateToShadowAndSmoke.md) | Small | 2 | 3 | 1 | 0 | The `kLightingTextures` stage of `RecreatePipelineGroups` now recreates both `kModel` pipelines (stale auto-appended `VkImageView`) and terrain-data pipelines (silent descriptor-pool churn → flat terrain). Same two bug classes still reachable from `kShadowTextures` (`mShadowBlurTexture`) and `kSmokeTextures` (`mSmokeTextureOne`). Extend both recreates to those flags via a single private helper plus an enlarged `bStageTerrainData` predicate. |
