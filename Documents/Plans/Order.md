...
|---|------|------|--------|--------|-------|-------|-------|
| 1 | [DataPacker/Refactor_ExportSceneDecompose.md](DataPacker/Refactor_ExportSceneDecompose.md) | Medium | 5 | 4 | 3 | 4 | Decompose `PreExport` (~265 lines) and `MainExport` (~280 lines) + one redundant-check simplification |
| 2 | [Network/Refactor_OversizedFunctions.md](Network/Refactor_OversizedFunctions.md) | Architectural | 5 | 4 | 3 | 4 | Decompose `ReconcileCoord` (207 lines), `WriteFleetData`/`ReadFleetData`, `ResetClientsForLoad`, `NewClients`, `Client::Poll`/`Server::Poll` sim/direct dedup |
| 3 | [DataPacker/Refactor_LongParamsAndFlags.md](DataPacker/Refactor_LongParamsAndFlags.md) | Medium | 4 | 3 | 3 | 4 | `Texture` bools → `Flags<TextureOptions>`, `LoadVertices`/`LoadAnimations`/`WriteBinding` param structs |
| 4 | [Network/Architecture_LayerViolationsPlanB.md](Network/Architecture_LayerViolationsPlanB.md) | Architectural | 4 | 3 | 3 | 4 | Layer-violations Plan B (Plan A landed): push `mCoordFrames` mutation/reads out of `ClientSessionBase`, add `ServerSessionBase` coord-state accessors, `CreateFrame()` factory, move the whole timespeed flow + frame-version seam to the game layer, `BufferFullFrame` takes pre-serialized bytes |
| 5 | [Graphics/Architecture_EngineCameraCoupling.md](Graphics/Architecture_EngineCameraCoupling.md) | Large | 5 | 3 | 2 | 4 | Inject `const engine::CameraBase&` (or a camera snapshot) into ~30 render/uniform/collection-render/audio sites across ~13 engine TUs instead of dereferencing the game-layer `game::gpCamera` global; add a seam for the game-only `kfCameraEyeHeightDefault` constant |
| 6 | [Graphics/IslandResidentMemoryScaling.md](Graphics/IslandResidentMemoryScaling.md) | Large | 4 | 3 | 3 | 4 | Per-template mesh + heightmap are permanently resident (not LRU'd); island 01's multi-island routes raised it to 65 templates (~9x the prior footprint). Measure the resident footprint, then bound it by concurrent residency (extend the texture LRU to meshes/heightmaps, or lazy-load the kIsland chunk) without breaking the record-once CB invariant; the `kiMaxIslands` 64->128 bump was the stopgap |
