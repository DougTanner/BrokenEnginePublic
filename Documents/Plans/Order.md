...
|---|------|------|--------|--------|-------|-------|-------|
| 1 | [Graphics/ShaderReview/04_Particles.md](Graphics/ShaderReview/04_Particles.md) | Medium | 4 | 5 | 3 | 2 | Billboard singularity on top-down RTS camera (`SquareParticlesRender.vert:50`, `LongParticlesRender.vert:53`); `ParticlesUpdate.comp:100` non-atomic RMW race on allocation bitmap (make `atomicAnd`); `ParticlesSpawn.comp` uint8/uint16 header mismatch; `ParticlesRender.frag:36` `pow` base clamp; double-integration on terrain bounce |
...
| 3 | [WrapperArrayCollapse.md](WrapperArrayCollapse.md) | Medium | 3 | 3 | 2 | 2 | Collapse 6 files of 4× per-target wrapper duplications into `Wrapper[kRenderTargetCount]` arrays indexed by enum. |
| 4 | [Engine/HeightLerpWrapperQuartet.md](Engine/HeightLerpWrapperQuartet.md) | Medium | 3 | 2 | 1 | 2 | Collapse 5 height-lerped `Wrapper` quartets (4 audio + 1 lighting = 20 globals) into `HeightLerpWrapperQuartet` struct + free `LerpAtHeight`; supersedes orphan `Graphics/CameraHeightLerpHelper.md`. |
| 5 | [Graphics/DynamicIslandLoadingFollowups.md](Graphics/DynamicIslandLoadingFollowups.md) | Medium | 3 | 3 | 2 | 2 | Two IslandTerrain/TextureManager follow-ups from Phase-5 dynamic-loading session: (1) reset `miTextureSlot`/`mbGpuResident` on device-lost TextureManager recreate — silently strands BOTH `kPipelineTerrainElevation` and `kPipelineShadowElevation` on placeholder otherwise; (2) wrap or document `AcquireTextureSlot` cold-path in `Islands::UpdateActiveIslands` for allocation tracker. |
| 6 | [Graphics/CoLocatePerSlotDescriptorRegistration.md](Graphics/CoLocatePerSlotDescriptorRegistration.md) | Medium | 3 | 3 | 2 | 2 | Co-locate per-slot bindless-array descriptor registration with the pipeline declaration in `PipelineManager.cpp` instead of hand-authoring it in `IslandTerrain::AcquireTextureSlot`. Eliminates the split-across-files bug class that caused commit `09fb128` to silently drop `kPipelineShadowElevation` registration; the next pipeline added against `mElevationTextures` will not need a new hand-authored line. |
