...
- **No allocation-tracking trips**: every `std::unordered_set` insert/find must be in a `ScopedSuppressAllocationTracking` scope. `LOG` itself must use the workbuffer formatters (`common::Wb`, `ScopedWorkbufferBuilder`) per project pattern; passing `std::string_view` directly to `LOG(..., "{}", key)` is allocation-safe.
