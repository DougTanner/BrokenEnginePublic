...
Manifest-accuracy caveat to state in the skill text: overlap detection is only as good as `## Critical files`; plan authors must keep that section complete (already a `Documents/Plans/CLAUDE.md` authoring rule — cross-reference it).
...
- `Documents/Plans/CLAUDE.md` — `[CLAIMED]` marker format (timestamp), authoring rule cross-reference for `## Critical files` completeness.
