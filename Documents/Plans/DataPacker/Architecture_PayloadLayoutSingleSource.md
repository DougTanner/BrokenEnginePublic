...
- Writer `ExportScene.cpp:530-537` and **three** readers: `Engine/Source/Graphics/Objects/ModelPipeline.cpp:38-43` (layout comment literally copy-pasted between the files), `Engine/Source/Graphics/Objects/PipelineDescriptorWriter.cpp:160-164` (third copy of the scene-arrays math — `RoundUp(textureCount*sizeof(crc_t)) + RoundUp(materialCount*sizeof(uint32_t))` to reach `MaterialShaderData`; found by the AnimationDataLoadPlacement sweep), and the animation-offset computation formerly at `Engine/Source/File/FileManager.cpp:363-367` (relocated into `Engine/Source/Graphics/AnimationData.cpp` by `File/Architecture_AnimationDataLoadPlacement.md` — re-locate the site at execution). Same approach on `common::SceneHeader`; an `AnimationSectionOffset(textureCount, materialCount)` helper covers the relocated site [~45m]
...
- `Engine/Source/Graphics/Managers/PipelineManager.cpp`, `Engine/Source/Graphics/Objects/ModelPipeline.cpp`, `Engine/Source/Graphics/Objects/PipelineDescriptorWriter.cpp`, `Engine/Source/Graphics/AnimationData.cpp` (scene animation-offset math, relocated here from `FileManager.cpp` by `File/Architecture_AnimationDataLoadPlacement.md`)
...
- A third scene-layout reader (the animation-section offset math, formerly `FileManager.cpp:363-367`, relocated by `File/Architecture_AnimationDataLoadPlacement.md` to `Engine/Source/Graphics/AnimationData.cpp:395-397` inside `LoadAnimationDataFromEagerChunks`) was missing from the original draft and has been added — it computes the animation-section offset with the same duplicated `RoundUp` chains and is exactly the site whose math `ExportScene.cpp:793`'s sanity check currently fails to mirror (see `Architecture_ChunkCacheVersioning.md`).
