...
1. **Verify the unreachability claim before editing** (per Diagnosis Discipline). Confirm against current source that `AddChecked` has exactly one call site (`FileManager.cpp:442`), that both its arguments derive from on-disk file sizes and the cluster size rather than from attacker- or config-controlled input, and that no other translation unit reaches this anonymous-namespace definition. If any of this is refuted — for example a second call site appeared, or a caller can drive `uiAllocation` from something other than real file sizes — stop and report rather than deleting.
2. Delete the `AddChecked` definition (`FileManager.cpp:179-186`).
3. Inline plain arithmetic at the sole call site (`:442`), preserving the existing cluster-rounding semantics exactly — round `uiSize` up to the next `uiClusterBytes` multiple, then accumulate into `uiAllocation`. Mirror the `DiagnosticReporter.cpp` precedent's shape.
4. Remove any `#include` or `using` that the deletion leaves unused (e.g. `<limits>`, if `std::numeric_limits` had no other use in the TU) — check before removing; do not remove headers still needed by other code.

## Critical files

- `DataPacker/Source/FileManager.cpp` — the anonymous-namespace `AddChecked` (`:179`) and its sole call site inside the output-materialization disk-space walk (`:442`)
...
- The unreachability verification result is recorded before the edit.
...
- `uiAllocation` at `FileManager.cpp:442` accumulates the identical cluster-rounded value as before for all non-overflowing inputs (i.e. every reachable input).
- DataPacker compiles.
