...
4. **Create a controlled small volume.** Have the fixture attach a small VHD for the case. Makes coverage host-independent in principle, but normally requires elevation, changes machine storage state, and adds cleanup failure modes to a fixture that currently only touches a temp directory.
