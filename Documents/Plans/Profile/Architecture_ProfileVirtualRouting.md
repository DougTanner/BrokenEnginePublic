...
## Verification Notes
- Checked against the root AGENTS.md engine-reads-game policy: this plan does NOT decouple/inject the sanctioned engine→game reads — the `static_assert` index contracts (`ProfileManagerBase.cpp:12-13`) and the FPS overlay's by-name `GetCpuTimer(game::kCpuTimerFrameUpdate)` read (`ProfileManagerBase.cpp:409`) are explicitly preserved. What collapses is only the virtual-accessor indirection (`ProfileManagerBase.cpp:428-456`), which the sanctioned direct-read policy makes unnecessary.
...
- Profile/AGENTS.md notes the accessors are deliberately outside `if constexpr (kbProfiling)` so the server GDI display renders zeros when profiling is off — the non-virtual replacements must keep that property (they do trivially).
