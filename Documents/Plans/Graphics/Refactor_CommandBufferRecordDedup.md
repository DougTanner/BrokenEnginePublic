...
### Tile-count helper
- Add a one-line `TileCount(uint32_t)` helper (GraphicsUtils or `shaders::`) and fold the ~16 hand-expanded `(w + shaders::kiComputeTileSize - 1) / shaders::kiComputeTileSize` expressions: `CommandBufferRecordGlobal.cpp:43-44,48-49,80-81`; `CommandBufferRecordMain.cpp:173,176,396-397,423-424`; `BufferManager.cpp:572-573,612-613`; `Render/WindUniforms.cpp:54-55`; `Render/SmokeUniforms.cpp:37-38`. Do NOT touch `BufferManager.cpp:660-661` (`CreateLightingSpreadBuffers` uses a different `std::max(1u, w / tile)` floor-div formula) or `Render/MainUniforms.cpp:353` (a `+ 1` variant — fold only if it reads cleanly as `TileCount(iQuadCountX + 1)`) [~30m]
