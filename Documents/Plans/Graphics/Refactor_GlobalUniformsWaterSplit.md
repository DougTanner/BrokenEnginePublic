...
- **Co-scheduling.** `GlobalUniforms.cpp` sits in a File Group with `WindowedLightingShadowDispatch`. That plan edits the shadow / lighting / temporal regions (earlier in the file) and is **disjoint** from the water block moved here — but this split shifts `GlobalUniforms.cpp` structure, so refresh its `GlobalUniforms.cpp` line citations if landed together. The shared `TemporalAreaLatch` in `Engine/Source/Graphics/Render/Render.h` does not touch the water accumulators, so there is no interaction with the moved statics.
