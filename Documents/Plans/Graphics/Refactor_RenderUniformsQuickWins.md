...
- `fSpreadDecayEnd`/`fSpreadAccumulationDecayEnd` (:157-158) are written from the *same* wrappers as the Start block (:142-143) — no `gSpreadDecayEnd`/`gSpreadAccumulationDecayEnd` wrapper exists (grep-verified), while every other End-block field has one; `LightingSpread.frag:67-68` interpolates Start→End for both — always a no-op. **Decision (2026-07-03): add the two End wrappers** — declare/define `gSpreadDecayEnd`/`gSpreadAccumulationDecayEnd` in `LightingWrappersBase.{h,cpp}` with defaults equal to the corresponding Start wrappers' defaults (visuals unchanged until an author moves a slider), wire them at :157-158, and add the two TweaksScreen sliders next to the other End controls. This restores the per-pass ramp the shader is built for via the established pattern every other End field already follows (10 sibling End wrappers at :150-161), touches no shader and needs no repack; collapsing instead would edit `LightingSpread.frag` + drop layout fields + repack, and would leave decay as the lone non-rampable spread parameter — more work for less capability [~15m]
...
- `f4RenderVisibleArea` expansion (:85-88): `.z`/`.w` recompute `(y − w)` *after* `.y` was expanded, so east/bottom use a larger height than west/top. **Decision (2026-07-03): confirmed bug — fix it.** Hoist `fHeight` before the `.y` expansion so all four edges use one height (~15m); tunables-calibration caveat stays as an execution-time verify + visual check
- ~~Optional: decompose the 143-line `CalculateMatricesAndVisibleArea`~~ — **skipped (YAGNI)**, out of scope
...
- The optional `CalculateMatricesAndVisibleArea` phase decomposition (skipped, YAGNI — see Decision below)

## Notes
- Invariant exposure: client/graphics-only; no determinism/CRC/wire. The L1 item changes visuals if the ramp is restored (that's the point) — shader repack either way; C2 is behavior-affecting (visible-area size fix)
- Decision (2026-07-03): (a) L1 — add the two End wrappers (see the L1 bullet; defaults mirror the Start wrappers so behavior is unchanged until tuned). (b) C2 — confirmed bug; hoist `fHeight` before the `.y` expansion (tunables-calibration verify at execution time). (c) optional CameraBase decomposition — skipped (YAGNI)
