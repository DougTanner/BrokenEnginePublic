...
- **Final-evidence wording remains compatible with large evidence references.** Narrow only the sentence that forbids intermediate evidence locators so it continues to reject proof-only envelopes, hashes, IDs, and dependency graphs while permitting an existing artifact or log path plus the selector needed to recover genuinely large evidence. Preserve the inline acceptance table, manifest, and `Build required` contract unchanged.
...
- Handoff report formats already defined in the named skills' "## Handoff" sections and the inline acceptance-table and manifest contract in `subagent-reporting.md` "Final-evidence gate".
...
- A semantic contract-consistency matrix across the root contract, canonical reporting reference, and specialized skills shows consistent fresh/none context defaults, the two required prompt records, the canonical bounded brief fields, manager routing, evidence references, and recovery capsules while preserving the inline final-evidence table, manifest, and `Build required` invariant; no temporary-file-only handoff statement survives in root `AGENTS.md`.
- An independent exact call-site and field-coverage sweep of the in-scope delegation regions finds fresh/none delegation with both prompt records and no omitted fixed-user-decision or governing-instruction/artifact brief field.
