...
- For each dangling citation, choose per instance and record the choice:
  ...
  - the referenced work is still real and unowned — say so in place of the dead path, and route it through `/create-follow-up-plans` so it gets an executable plan rather than a prose promise;
...
- **Sizing.** The real shape is ~18 area-prefixed instances in 11 files against 13 absent targets, plus ~30 bare-name instances — still mechanical prose edits, but each distinct absent target needs its own done / still-real / historical determination.
...
- Plan-body prose is an ordinary tracked edit per `Documents/Plans/AGENTS.md`; preserve executable metadata when updating it.
