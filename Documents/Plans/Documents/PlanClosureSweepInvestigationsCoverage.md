...
The mechanism that reports references to a Plan being completed does not see it. `.agents/scripts/Find-PlanClosureReferences.ps1:38` walks exactly `@('Documents/Plans', 'Documents/Features')`; everything else in the script — the full-path, area-relative, and bare-name term derivation (`:22-35`) and the `broken-engine-plan-closure-references/v1` emission (`:56`) — is tree-agnostic. `Documents/Plans/Documents/StalePlanCrossReferences.md:19` and `:40` name this script as the reuse mechanism for the repo-wide dangling-citation sweep, so the blind spot propagates into that sweep's coverage as well.
...
4. If `Documents/Plans/Documents/StalePlanCrossReferences.md` is still present, correct its one descriptive sentence at `:19` ("it scans both planning trees") to name three. That Plan's metadata marker, scope contract, and every other sentence stay byte-for-byte unchanged.
...
- `Documents/Plans/Documents/StalePlanCrossReferences.md` — the single descriptive sentence at `:19`, and only if that Plan is still present.
