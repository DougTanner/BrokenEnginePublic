...
1. **`ServerSession::PreTickNetwork` (`ServerSession.cpp:264`):** skip `mpBroadcaster->ClearPendingRequests()` while paused so `mPendingUpdatePlayerRequests` survives. (`mpFleetManager->ClearPendingRequests()` on the next line is a different queue family — network fleet CRUD requests refilled each poll — leave it as is.)
2. **`ServerBroadcaster::BuildFrameInputs` (`ServerBroadcaster.cpp:17-98`):** cache `const bool bPaused = ...` once, then wrap the injection work in two `if (!bPaused)` blocks that preserve current statement order around the unconditional `TickFleetTimers()` (`:77`, must keep running every cycle — it is `mfLastDeltaTime`-scaled and self-quiescent during pause, per `Server/CLAUDE.md`):
   - Block 1 (before `TickFleetTimers`): the waiting-spawn loop (`:31-50`), the pending-destroy loop + its clear (`:52-65`), and `ProcessUpdatePlayerRequests()` (`:68`).
   - Block 2 (after `TickFleetTimers`): `ProcessFlagshipUpdates()` (`:78`), the `mSpawns` save loop (`:80-87`), and the pre-spawn snapshot refresh (`:89-97`).
...
- The vestigial `mPendingPlayerDestroys` queue (`ServerClientManager.h:39`) — it has **no fill site** in the current codebase (only iteration/clear at `ServerBroadcaster.cpp:53,65`, `ServerSession.cpp:312`, `ServerClientManager.cpp:345`); its loop lands inside Block 1 as-is, but deleting the dead queue is a separate cleanup, not this plan.
