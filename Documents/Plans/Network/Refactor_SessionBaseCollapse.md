...
- `GameBase.cpp:62` `game::gpClientSession->GetTargetSimTick()` and `:108` `game::gpServerSession->WaitForTick(mTimeStep)` — `GameBase.cpp` already `#include`s `Network/Client/ClientSession.h` / `Network/Server/ServerSession.h` under `BT_CLIENT`/`BT_SERVER` (`:4-9`). Methods are inherited today; they become direct members after collapse.
