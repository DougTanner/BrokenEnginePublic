...
Priority index — single table of all live plans roughly ordered by score, lowest first (Score = Effort − Impact + Risks) — the order is a loose tiebreaker for what to pick next, a rough estimate rather than a strictly maintained invariant. It reflects only the CURRENT set of live plans; **it is not a changelog.** Do not record landed/dropped plans, "this run / this session" retrospectives, "has landed and been removed" annotations, or review-sweep narratives here — every row and reference must correspond to a plan file that exists on disk right now. It also carries:
...
Plan coordination is AgentCli state, never `Order.md` content. Use `Tools\AgentCli\Platforms\VisualStudio2026\Output\AgentCli.exe` from the current checkout, generate an owner with `lock token`, and address this queue with canonical Git common directory plus repo-relative `Documents/Plans/Order.md`. Linked worktrees consume the provisioned immutable primary Output; routine workflows never build AgentCli or write through that link. Queue mutations and selection run under `plan queue lock --repo <git-common-dir> --order Documents/Plans/Order.md --owner <token> --session <label> --worktree <path>`; the returned claims snapshot is authoritative for that selection attempt. `plan queue list|status|steal|unlock` use the same queue locator, with owner/expect/session/worktree arguments as required. Plan claims use `plan row claim|status|steal|unclaim --repo <git-common-dir> --order Documents/Plans/Order.md --plan <Order-relative-plan>`; claim and steal require ownership of the matching queue lock, and unclaim requires the row owner. Claim age is warning-only. Takeover requires explicit user approval and conditional row steal using the owner token returned by fresh status. Never place claim state or owner/session metadata in `Order.md` or a plan file.
...
- Insert the new row at roughly its score-sorted position (exact placement isn't important — the order is a loose tiebreaker). If the plan touches files already listed in `## File Groups` or `## Dependencies`, add it to the relevant entry there too.
