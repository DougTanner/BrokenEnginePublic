...
WorktreeCli is the only scheduler parser and claim mutator. It selects the oldest eligible executable plan by `(createdUtc, canonical path)`. Existing valid dependencies block a child; a missing dependency is a satisfied stale edge reported as a notice. Invalid metadata and dependency cycles quarantine only the affected plans, so unrelated plans remain claimable. To quarantine a plan is to exclude it from selection because its metadata is invalid or its dependencies form a cycle, without affecting other plans.
