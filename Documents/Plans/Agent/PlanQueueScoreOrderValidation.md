...
`Documents/Plans/Order.md` promises one `## Plans` table sorted by numeric `Score`, and `/next-plan` selects by walking that table top-down. The live table violates that invariant in one place: `Graphics/Managers/Refactor_PipelineManagerSplit.md` has score 1 sitting inside the score-3 block. Its score arithmetic is correct and the linked file exists; its row position is wrong. As a result, an unclaimed higher-priority plan can be skipped behind lower-priority work.
...
- `Graphics/Managers/Refactor_PipelineManagerSplit.md` appears in the score-1 block; all equal-score rows retain their pre-repair relative order.
