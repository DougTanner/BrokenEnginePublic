...
- `Projects/BrokenEngineSandbox/Source/Ui/AGENTS.md`, `Projects/BrokenEngineSandbox/Source/AGENTS.md` (doc updates for site a)
...
- The two deliberately-unwrapped wrappers `ParticleWrappers` / `WindDepositsWrappers` (both-vcxproj by design — unguarded server frame code references their symbols; `Ui/AGENTS.md`).
...
- **Site b**: now a documented-accepted exception (`Server/AGENTS.md:14`), not actionable — see the Design bullet for the surfaced contradiction. `Network/Refactor_SessionBaseCollapse.md` deletes `ServerSessionBase.*` outright anyway, so the partial guard disappears when that lands.
