...
2. Choose one enforcement contract:
   ...
   - **B — deterministic diagnostic scan-and-fail:** add one compile-skill-owned wrapper that runs the serialized WorktreeCli build, preserves live output and the original MSBuild exit code, recognizes canonical Microsoft analysis diagnostic records, and returns nonzero when the selected policy matches. Keep WorktreeCli's generic build command unchanged unless inspection proves the contract cannot be implemented reliably at the skill boundary; a WorktreeCli change rebuilds through AgentTools candidate production and session-landing promotion (see `/compile`). The wrapper consumes the `broken-engine-build-result/v1` structured diagnostics rather than scraping console text.
...
- `Tools/WorktreeCli/BuildCommand.cpp` — conditional only if option B cannot preserve output and exit semantics at the compile-skill boundary.
...
- Generalizing WorktreeCli into a diagnostic-policy framework beyond this explicit PREfast contract.
