...
- Evaluate and, after explicit ThirdParty approval, replace the manual usage/argument/dispatch ranges in `AgentHarness.cpp:27-33,118-184,289-311`, `HarnessLockCommands.cpp:40-84,96-143`, `WorktreeCli.cpp:15-73`, `LandingLockCommands.cpp:50-105,109-153`, `PlanCommands.cpp:42-88,500-535`, `PlanOrderCommands.cpp:176-244,2371-2431`, and `BuildCommand.cpp:656-683` with CLI11. The measured candidate spans total 4,570 bt-token-v1; retain semantic validation and dispatch, targeting approximately 2,500-3,000 bt-token-v1 net removal. CLI11 is header-only under BSD-3-Clause, which is on the repository allow-list, and is not currently present under `ThirdParty/`. [~1h]
...
- Replacing process execution, Git plumbing, JSON, coordination storage, queue parsing, MSBuild diagnostics, or harness framing.
...
## Notes
- Invariant exposure: AgentHarness/WorktreeCli command-line protocol and build/queue operational contracts; no engine determinism/CRC, `.pack`, replay, client/server, or allocation-tracked runtime exposure.
