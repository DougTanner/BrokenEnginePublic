...
8. Treat this client-only UI work as Tier 2 unless its final scope exposes a higher-risk invariant. Use the task-driven workflow: propagate only required signature, semantics, guard, or mirrored effects; run targeted compilation and the native-resolution harness scenarios below; and use one C++ correctness review plus the shader specialist only if shader bytes change. Apply style, documentation, project-membership, adversarial, and whole-change audit work only when their concrete triggers apply. The approved native-resolution harness scenarios are the decisive runtime evidence. Keep any in-scope structural acceptance failure blocking this pass; route only proven pre-existing or out-of-scope structural residuals to separate plan files.
