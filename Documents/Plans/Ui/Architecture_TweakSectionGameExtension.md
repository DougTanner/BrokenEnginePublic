...
The engine `TweakSection` enum (`Engine/Source/Ui/Screens/TweaksScreen/TweaksScreenBase.h:8-23`) names game-only sections directly — `kHexShield` (:18) and `kParticles` (:21), mirrored in `TweakSectionFlags` (:38, :41). This is the root-CLAUDE.md "reverse direction" violation (an engine *type* naming game concepts), unlike `engine::PacketType`'s `kGamePacketStart` opaque extension point. Game consumers: `Projects/BrokenEngineSandbox/Source/Ui/Screens/TweaksScreen/TweaksScreenHexShield.cpp:33`, `TweaksScreenParticles.cpp:64`.
