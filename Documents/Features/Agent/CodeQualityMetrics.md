...
- Compare mode archives the fixed baseline and separately captures current tracked plus untracked/nonignored source into an immutable temporary tree. Record path, mode, and SHA-256 identities before and after capture; exit `2` on manifest or content drift.
...
- Match functions within baseline/current file pairs by undecorated owner, name, and signature only when unique on both sides. Report overloads or duplicate identities as ambiguous/unmatched without directional claims. Coverage/profile/identity changes suppress total-score “improved/regressed” labels while retaining raw and common-cohort evidence.
...
- Add Compare to `$repo-code-review` using its fixed baseline and immutable changed-file manifest. Regression alone never changes PASS or becomes a finding. Only independent source inspection can promote a substantial new near-copy through the existing duplication rule or prove another existing correctness violation. Structural-erosion changes remain advisory/follow-up evidence. Operational failure makes the review incomplete, not a correctness finding.
...
- Run scenario checks without adding unit tests:
  ...
  - Extended profile includes `.h`; strict profile excludes it; cross-profile comparison fails.
