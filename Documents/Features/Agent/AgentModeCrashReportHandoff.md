...
4. Add `.agents/skills/agent-harness/scripts/Invoke-AgentHarnessProcessCheck.ps1` as the surviving launch-health owner. Its launch operation records each exact PID, process role, launch time, both report paths, and each report's pre-launch existence/size/last-write identity in the run evidence directory. Its check operation returns one typed result covering PID liveness, intentional-stop state, transport loss, and whether either report changed after launch.
...
- `.agents/skills/agent-harness/scripts/Invoke-AgentHarnessProcessCheck.ps1` — typed launch baseline, intentional-stop marker, liveness/report comparison, and notification evidence.
