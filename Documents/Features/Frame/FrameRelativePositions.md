...
**D5 — Version bump.** Positions serialize as raw floats whose meaning changes → bump the `Frame::kiVersion` base constant (`Frame.cpp:11`) by +1 **on top of** whatever base `Plans/Frame/CrcVersionGateBump.md` has landed (it takes 115→116; this plan takes the result +1). Network full-state, saves, and replays are all gated by the same constant — no other serialization change needed. `FrameStaticData` serialization (`vecArea`, placement `f2WorldPos`) is untouched — those stay world-space.
