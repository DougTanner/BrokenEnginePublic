...
| [Agent/WorktreeRuntimeDataDirectory.md](Agent/WorktreeRuntimeDataDirectory.md) | Medium | 3 | 4 | 1 | 0 | Two explicit worktree data modes: ordinary code builds validate/use authoritative generated headers + packs without exporter writes; data/export builds produce and test isolated local outputs. Adds shared client/server `--data-directory`, compile/harness routing, and read-only compatibility validation |
...
- **`Engine/Source/File/FileManager.{h,cpp}`**: `Agent/WorktreeRuntimeDataDirectory.md` selects an explicit startup data root. Cross-queue overlaps: `Documents/Plans/File/Architecture_FileManagerSplitDecision.md`, `FileManagerReduceFile.md`, `EagerPackBufferReclaim.md`, `Documents/Plans/Graphics/TextureChunkCpuPoolReclaim.md`, `IslandHeightmapRouteDedup.md`, and `Documents/Plans/Network/PackIntegrityHandshake.md`. No prerequisite; later lander refreshes placement/citations and reverifies startup loading.
- **Game client/server vcxprojs**: `Agent/WorktreeRuntimeDataDirectory.md` changes generated-header/custom-build property sections; `Graphics/DecalSystem.txt`, `Engine/GrassRendering.txt`, and `Graphics/HeatDistortionAndShockwave.txt` plus cross-queue `Documents/Plans/Network/Refactor_SessionBaseCollapse.md` change source/shader membership. Regions are independent; later lander reconciles XML and reruns both builds.
- **`.agents/skills/agent-harness/SKILL.md`**: `Agent/WorktreeRuntimeDataDirectory.md` changes launch setup/data-root reporting; `Agent/AgentQueryGlobalState.md` extends command documentation. Disjoint sections; later lander preserves both and runs harness verification.
