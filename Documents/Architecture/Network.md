...
- Each Tick is simulated exactly once: previous Frame Tick(`sharedCrc`) + StatusChanges → new Frame Tick(`sharedCrc`). A second `inputCrc` is tracked server-side for debugging input determinism but is not validated against the client
...
- **Mismatch path (rollback + replay)**: when the oldest un-confirmed Tick's `sharedCrc` disagrees with the server's:
  1. Rollback — normally to the tick *before* the mismatch (**shrunk rollback**), not all the way to `iConfirmedTick`, to minimize re-sim cost. If shrunk-rollback replay fails at its first tick, the system falls back to a **full rollback** to `iConfirmedTick`
