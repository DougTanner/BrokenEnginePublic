...
- `Architecture/` — per-subsystem source of truth: Mermaid diagrams (`FrameUpdatePipeline.md`, `GameReconciliation.md`) and a prose protocol spec with exact constant values (`Network.md`). The `update-architecture-diagrams` skill keeps these current; AGENTS.md docs link to them rather than duplicating their content.
