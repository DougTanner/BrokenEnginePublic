...
Plan/intent: <WORKTREE>\Documents\Plans\Engine\ReleaseStaticAnalysisWarningCleanup.md — approved explicit Microsoft PREfast verification, process step 8

# Overall status

NEEDS_ACTION. Provisioning and all lifecycle/data preconditions passed, then both requested foreground WorktreeCli Release PREfast commands completed with final exit code 1. Both failed in the project pre-build provisioning target before C++ compilation or Microsoft code analysis because Windows PowerShell's loaded framework does not provide `System.Security.Cryptography.SHA256.HashData`.
...
- Routine provisioning command succeeded before building: `.agents\scripts\Provision-WorktreeThirdParty.ps1 -RepositoryRoot <ROOT>`.
- Immutable prebuilt WorktreeCli: `<WORKTREE>\Tools\WorktreeCli\Platforms\VisualStudio2026\Output\WorktreeCli.exe`.
- Changed-path set used for data-mode selection: `.agents/skills/compile/SKILL.md`; `Common/Log/Log.h`; `Documents/Plans/Engine/ReleaseStaticAnalysisWarningCleanup.md`; `Documents/Plans/Order.md`; `Engine/Source/File/PackChunks.cpp`; `Engine/Source/Graphics/Managers/ImGuiManager.cpp`; `Engine/Source/Profile/ProfileManagerBase.cpp`.
- Local-data triggers: none. The compile skill change, plan files, and four changed C++ sources do not match any Local trigger.
...
- The Shared directory is absolute and outside the worktree.
- Required primary data completeness: PASS; all 10 required headers and all 16 required manifest/pack files exist and are nonempty.

# Selected primary-data identity snapshot
...
- Snapshot before first build: recorded above.
- Snapshot after client command: IDENTICAL (26/26 required files; same relative paths, lengths, and SHA-256 values).
- Snapshot after server command: IDENTICAL (26/26 required files; same relative paths, lengths, and SHA-256 values).
...
- Complete observed output, including every error line verbatim:
...
- C6011: absent from both command outputs, but not conclusively verified because compiler analysis did not start.
- C6250: absent from both command outputs, but not conclusively verified because compiler analysis did not start.
- C5054: absent from both command outputs, but not conclusively verified because compiler analysis did not start.
- C6385: absent from both command outputs, but not conclusively verified because compiler analysis did not start.
- C6001: absent from both command outputs, but not conclusively verified because compiler analysis did not start.

# Decision index

- B001 | blocker | `.agents/scripts/Provision-WorktreeThirdParty.ps1` under project pre-build target | Windows PowerShell runtime lacks `SHA256.HashData`; client and server stop before C++ compilation/PREfast.
- R001 | residual | client Release PREfast | Required build failed with final exit code 1; warning-cleanup verification remains incomplete.
- R002 | residual | server Release PREfast | Required build failed with final exit code 1; warning-cleanup verification remains incomplete.
...
- C6011, C6250, C5054, C6385, and C6001 were not emitted, but absence cannot be verified until PREfast runs.
