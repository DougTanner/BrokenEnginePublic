...
function Get-AgentCliRepositoryIdentity([string] $RepositoryRoot) {
	...
	return [pscustomobject]@{
		Repository = $common
		LedgerPath = Join-Path $directory "agentcli-sessions-$hash.json"
		MutexName = "Global\BrokenEngine_AgentCli_${sid}_$hash"
	}
}
...
function Read-AgentCliLedger($Identity) {
	...
	if ($ledger -isnot [pscustomobject] -or $ledgerNames.Count -ne 4 -or $missingLedgerNames.Count -ne 0 -or
		...
		throw "AgentCli session ledger failed validation: '$($Identity.LedgerPath)'."
	}
	...
	foreach ($claim in @($ledger.sessions)) {
		if (-not (Test-StrictClaim $claim $Identity $owners)) {
			throw "AgentCli session ledger contains an invalid session claim: '$($Identity.LedgerPath)'."
		}
	}
	if ($null -ne $ledger.maintenance) {
		...
		if (-not (Test-StrictClaim $m $Identity $null) -or $owners.Contains($m.owner) -or @($ledger.sessions).Count -ne 0) {
			throw "AgentCli session ledger contains an invalid maintenance claim: '$($Identity.LedgerPath)'."
		}
	}
	return $ledger
}
...
function Write-AgentCliLedger($Identity, $Ledger) {
...
function Invoke-LedgerTransition($Identity, [scriptblock] $Action, [DateTime] $Deadline = [DateTime]::UtcNow.AddSeconds($script:DefaultWaitSeconds)) {
	...
	try {
		...
		$ledger = Read-AgentCliLedger $Identity
...
function Initialize-AgentCliLedger($Identity, [switch] $LegacySessionsClosed) {
	if (-not $LegacySessionsClosed) {
		throw 'AgentCli exclusion ledger is not initialized. Confirm all legacy pre-protocol sessions are closed, then retry with -LegacySessionsClosed.'
	}
...
function Register-AgentCliSession {
	...
	do {
		$result = Invoke-LedgerTransition $identity {
			param($ledger)
			if ($null -eq $ledger) { $ledger = Initialize-AgentCliLedger $identity -LegacySessionsClosed:$LegacySessionsClosed }
			...
			if ($bootstrapMissing) {
				if ($null -eq $ledger.maintenance -and @($ledger.sessions).Count -eq 0) {
					...
					Write-AgentCliLedger $identity $ledger
					return 'maintenance'
				}
				Write-Host 'Waiting for elected AgentCli bootstrap maintenance to complete.'
				return $null
			}
			if ($null -eq $ledger.maintenance) {
				if (@($ledger.sessions | Where-Object { $_.owner -eq $Owner }).Count -ne 0) { throw "AgentCli session owner '$Owner' already exists." }
				$ledger.sessions = @($ledger.sessions) + (New-Claim $Owner $Label $identity.Repository $Worktree)
				Write-AgentCliLedger $identity $ledger
				return 'session'
			}
			Write-Host "Waiting for AgentCli maintenance owner=$($ledger.maintenance.owner) label=$($ledger.maintenance.label) worktree=$($ledger.maintenance.worktree)."
			return $false
		} $deadline
		if ($result) { return [pscustomobject]@{ Owner = $Owner; Identity = $identity; Mode = $result } }
		if ([DateTime]::UtcNow -ge $deadline) { throw "Timed out after $WaitSeconds seconds waiting for AgentCli maintenance exclusion." }
...
function Get-AgentCliExclusionStatus {
...
function Assert-AgentCliSessionOwner([string] $RepositoryRoot, [string] $Owner) {
	...
	Invoke-LedgerTransition $identity {
		param($ledger)
		if ($null -eq $ledger) { throw 'AgentCli exclusion ledger is not initialized.' }
		...
		if ($claim.Count -ne 1 -or -not (Test-ClaimProcessLive $claim[0])) {
			throw "AgentCli session owner '$Owner' is not live."
		}
		Write-AgentCliLedger $identity $ledger
	}
}
...
function Unregister-AgentCliSession([string] $RepositoryRoot, [string] $Owner) {
	...
	Invoke-LedgerTransition $identity {
		...
		if ($null -eq $ledger) { throw 'AgentCli exclusion ledger is not initialized.' }
		$claim = @($ledger.sessions | Where-Object { $_.owner -eq $Owner })
		if ($claim.Count -ne 1 -or $claim[0].pid -ne $PID -or -not (Test-ClaimProcessLive $claim[0])) { throw "AgentCli session release owner mismatch for '$Owner'." }
		$ledger.sessions = @($ledger.sessions | Where-Object { $_.owner -ne $Owner })
		Write-AgentCliLedger $identity $ledger
	}
}

function Enter-AgentCliMaintenance {
		...
		$result = Invoke-LedgerTransition $identity {
			...
			if ($null -eq $ledger.maintenance -and $otherSessions.Count -eq 0) {
				if ($UpgradeSession) {
					...
					if ($own.Count -ne 1 -or $own[0].pid -ne $PID) { throw "Cannot upgrade non-owned AgentCli session '$Owner'." }
					$ledger.sessions = @()
				}
			...
			Write-Host "Waiting for AgentCli sessions [$owners] or maintenance [$maintenanceOwner]."
			return $false
		} $deadline
		if ($result) { return [pscustomobject]@{ Owner = $Owner; Identity = $identity; Upgraded = [bool]$UpgradeSession } }
		if ([DateTime]::UtcNow -ge $deadline) { throw "Timed out after $WaitSeconds seconds waiting for exclusive AgentCli maintenance." }
		$remainingMilliseconds = [Math]::Max(0, [Math]::Floor(($deadline - [DateTime]::UtcNow).TotalMilliseconds))
		if ($remainingMilliseconds -gt 0) { Start-Sleep -Milliseconds ([Math]::Min(5000, $remainingMilliseconds)) }
	} while ($true)
}

function Exit-AgentCliMaintenance([string] $RepositoryRoot, [string] $Owner, [switch] $DowngradeToSession, [string] $Label, [string] $Worktree) {
	$identity = Get-AgentCliRepositoryIdentity $RepositoryRoot
	Invoke-LedgerTransition $identity {
		param($ledger)
		if ($null -eq $ledger -or $null -eq $ledger.maintenance -or $ledger.maintenance.owner -ne $Owner -or
			...
			throw "AgentCli maintenance release owner mismatch for '$Owner'."
		}
...
function Invoke-AgentCliTrackedProcess {
