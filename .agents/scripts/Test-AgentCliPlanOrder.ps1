...
$script:FixtureRoot = Join-Path $script:RepositoryRoot "Temp/AgentCliPlanOrderFixtures/$([Guid]::NewGuid().ToString('N'))"
...
function Invoke-AgentCli([string[]] $Arguments, [int] $ExpectedExitCode) {
	Register-PlanOrderQueueLocks $Arguments
	$result = Invoke-Process -FilePath $script:Executable -Arguments $Arguments
	if ($result.ExitCode -ne $ExpectedExitCode) {
		throw "AgentCli $($Arguments -join ' ') exited $($result.ExitCode), expected $ExpectedExitCode.`nstdout: $($result.Stdout)`nstderr: $($result.Stderr)"
	}
	return $result
}
...
function ConvertFrom-AgentJson($Result) {
	...
	catch { throw "AgentCli stdout was not one JSON value:`n$($Result.Stdout)`nstderr:`n$($Result.Stderr)" }
}
...
function New-FixtureRepository(
	...
	Invoke-Git $primary @('config', 'user.email', 'agentcli-fixture@example.invalid') | Out-Null
	Invoke-Git $primary @('config', 'user.name', 'AgentCli Fixture') | Out-Null
...
function Get-Validation($Fixture, [string] $Worktree, [int] $ExpectedExitCode = 0) {
	$result = Invoke-AgentCli (Get-OrderArguments $Fixture 'validate' $Worktree) $ExpectedExitCode
...
function Invoke-QueueLock($Fixture, [string] $Order, [string] $Owner, [string] $Session, [string] $Worktree) {
	...
	return Invoke-AgentCli @('plan', 'queue', 'lock', '--repo', $Fixture.Common, '--order', $Order, '--owner', $Owner, '--session', $Session, '--worktree', $Worktree) 0
}

function Invoke-QueueUnlock($Fixture, [string] $Order, [string] $Owner) {
	Invoke-AgentCli @('plan', 'queue', 'unlock', '--repo', $Fixture.Common, '--order', $Order, '--owner', $Owner) 0 | Out-Null
}
...
function Invoke-RowClaim($Fixture, [string] $Order, [string] $Plan, [string] $Owner, [string] $Session, [string] $Worktree) {
	...
	try {
		Invoke-AgentCli @('plan', 'row', 'claim', '--repo', $Fixture.Common, '--order', $Order, '--plan', $Plan, '--owner', $Owner, '--session', $Session, '--worktree', $Worktree) 0 | Out-Null
		$status = ConvertFrom-AgentJson (Invoke-AgentCli @('plan', 'row', 'status', '--repo', $Fixture.Common, '--order', $Order, '--plan', $Plan, '--owner', $Owner) 0)
...
function Invoke-RowUnclaim($Fixture, [string] $Order, [string] $Plan, [string] $Owner) {
	...
	Invoke-AgentCli @('plan', 'row', 'unclaim', '--repo', $Fixture.Common, '--order', $Order, '--plan', $Plan, '--owner', $Owner) 0 | Out-Null
}
...
function Invoke-ClaimNext($Fixture, [string] $Worktree, [string] $Owner, [string] $Queue, [int] $ExpectedExitCode, [string] $Plan = '') {
	...
	$result = Invoke-AgentCli $arguments $ExpectedExitCode
...
if ($executableItem.PSIsContainer -or ($executableItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -or $executableItem.Length -eq 0) {
	throw "AgentCli executable must be a nonempty ordinary file: '$script:Executable'."
}
$help = Invoke-AgentCli @('--help') 0
Assert-True $help.Stdout.Contains('AgentCli.exe plan order validate', [StringComparison]::Ordinal) 'AgentCli does not advertise plan order; build it before running this fixture.'
...
try {
	...
	Invoke-Case 'add independent sequences and prerequisite chains atomically' {
		...
		$result = Invoke-AgentCli ((Get-OrderArguments $fixture 'add' $session) + @('--owner','add-owner','--session','add-session','--request',$requestPath)) 0
		$json = ConvertFrom-AgentJson $result
		...
		Invoke-AgentCli ((Get-OrderArguments $fixture 'add' $session) + @('--owner','bad-owner','--session','bad-session','--request','Temp/malformed.json')) 1 | Out-Null
	...
	Invoke-Case 'update opaque bytes and reject a claimed multi-update counterpart' {
		...
		$json = ConvertFrom-AgentJson (Invoke-AgentCli ((Get-OrderArguments $fixture 'update' $session) + @('--owner','update-owner','--session','update-session','--request',$requestPath)) 0)
		...
		try {
			...
			$conflict = ConvertFrom-AgentJson (Invoke-AgentCli ((Get-OrderArguments $fixture 'update' $session) + @('--owner','blocked-owner','--session','blocked-session','--request',$blockedPath)) 2)
	...
	Invoke-Case 'claim concurrency, claimed prerequisite blocking, and owner mismatch' {
		...
		$mismatch = ConvertFrom-AgentJson (Invoke-AgentCli ((Get-OrderArguments $fixture 'complete' $one) + @('--owner','wrong-owner','--session','wrong-session','--plan','Documents/Plans/A.md')) 2)
	...
	Invoke-Case 'complete prunes edges, reapplies idempotently, and rolls back exact bytes' {
		...
		$result = ConvertFrom-AgentJson (Invoke-AgentCli ((Get-OrderArguments $fixture 'complete' $session) + @('--owner','complete-owner','--session','complete-session','--plan','Documents/Plans/A.md')) 0)
		...
		$reapply = ConvertFrom-AgentJson (Invoke-AgentCli ((Get-OrderArguments $fixture 'complete' $session) + @('--owner','complete-owner','--session','complete-session','--plan','Documents/Plans/A.md','--reapply')) 0)
		...
		try { Invoke-AgentCli ((Get-OrderArguments $rollbackFixture 'complete' $rollbackSession) + @('--owner','rollback-owner','--session','rollback-session','--plan','Documents/Plans/A.md')) 1 | Out-Null }
	...
	Invoke-Case 'stale session cannot reclaim after completion lands' {
		...
		Invoke-AgentCli ((Get-OrderArguments $fixture 'complete' $worker) + @('--owner','landing-owner','--session','landing-session','--plan','Documents/Plans/A.md')) 0 | Out-Null
	...
	Invoke-Case 'finalize landing holds landing then queues across primary advance and unwinds failures' {
		foreach ($injection in @('pre-advance', 'post-advance')) {
			...
			try {
				Invoke-AgentCli @('lock','claim','--domain','landing','--repo',$fixture.Common,'--owner',$owner,'--session',$session,'--worktree',$advance,'--lease-seconds','3600') 0 | Out-Null
				$landingHeld = $true
				$events.Add('claim-landing')
				$landingStatus = ConvertFrom-AgentJson (Invoke-AgentCli @('lock','status','--domain','landing','--repo',$fixture.Common) 0)
				...
				foreach ($order in @('Documents/Features/Order.md', 'Documents/Plans/Order.md')) {
					...
					$status = ConvertFrom-AgentJson (Invoke-AgentCli @('plan','queue','status','--repo',$fixture.Common,'--order',$order,'--owner',$owner) 0)
				...
				$blockedBefore = ConvertFrom-AgentJson (Invoke-AgentCli $claimArguments 2)
				...
				$staleAfter = ConvertFrom-AgentJson (Invoke-AgentCli $claimArguments 2)
				...
				if ($landingHeld) {
					...
					Invoke-AgentCli @('lock','release','--domain','landing','--repo',$fixture.Common,'--owner',$owner) 0 | Out-Null
			...
			foreach ($order in @('Documents/Features/Order.md', 'Documents/Plans/Order.md')) {
				$status = ConvertFrom-AgentJson (Invoke-AgentCli @('plan','queue','status','--repo',$fixture.Common,'--order',$order) 2)
				Assert-True (-not $status.held) "The $injection safe-stop retained queue '$order'."
			}
			$landingStatus = ConvertFrom-AgentJson (Invoke-AgentCli @('lock','status','--domain','landing','--repo',$fixture.Common) 2)
	...
	Invoke-Case 'queue-lock contention fails without repository mutation' {
		...
		try {
			...
			$claim = ConvertFrom-AgentJson (Invoke-AgentCli $arguments 2)
			Assert-Equal $claim.reason 'already-locked' 'Contended claim did not report the held queue lock.'
			$featuresStatus = ConvertFrom-AgentJson (Invoke-AgentCli @('plan','queue','status','--repo',$fixture.Common,'--order','Documents/Features/Order.md') 2)
...
if ($script:Failures.Count -ne 0) {
	...
	throw "$($script:Failures.Count) AgentCli plan-order fixture case(s) failed."
}
$summary | ConvertTo-Json -Depth 10
