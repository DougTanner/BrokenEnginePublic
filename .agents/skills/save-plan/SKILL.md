...
5. **Save and index**: Write the plan content to `<tree>/<area>/<filename>`. Then read the destination tree's AGENTS.md (`Documents/Plans/AGENTS.md` or `Documents/Features/AGENTS.md`) and follow its rules: add the fully scored row (`Tier`/`Effort`/`Impact`/`Risks`/`Score`/`Notes`) to that tree's `Order.md` at its sorted position now — both trees forbid leaving a plan file unindexed or unscored.
