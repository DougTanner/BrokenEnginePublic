---
name: save-plan
description: Saves the current plan-mode plan from the user's home `.claude/plans/` directory into the repo's planning tree — `Documents/Plans/` for refactors/bugfixes or `Documents/Features/` for new capabilities — as a PascalCase `.md` file in the correct area subdirectory, then asks AgentCli to add its scored queue row from a structured request. Optional argument overrides the filename (e.g., `/save-plan MyPlan`).
...
7. **Create the add request**: Write a uniquely named JSON request beneath the current session worktree's `Temp/`. Use schema version 1 and exactly one independent sequence containing one entry:
   ...
   Use `queue: "features"` and a `Documents/Features/...` plan identity for the feature tree. Omit `dependsOn` when empty. Do not supply `score`; AgentCli computes `Effort - Impact + Risks`.

8. **Ask AgentCli to index it**: Use the current checkout's provisioned `Tools/AgentCli/Platforms/VisualStudio2026/Output/AgentCli.exe`, the canonical Git common directory, registered session checkout, an owner token generated by AgentCli `lock token`, the wrapper/session label, and the request's `Temp/` repository-relative path:
   ...
   Let AgentCli validate both queues, compute the score, acquire the existing queue locks, and add the row. Never edit an executable Order row directly. If add fails, report its exact diagnostics and leave the plan plus request as retryable session orphans; do not claim the plan was indexed and do not try to repair `Order.md` by hand.

9. **Report**: Confirm the saved repository-relative path, request path, destination queue, scoring inputs, dependencies, and AgentCli add receipt.
