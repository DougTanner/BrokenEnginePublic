...
3. **Reconcile automatically by default; escalate (lock-free) only when a safety check cannot clear it.** When primary has advanced, rebasing the session onto it is the routine multi-agent case — perform it automatically, without a sign-off round-trip, once BOTH mandatory safety checks below clear. Routine outcomes resolve automatically: disjoint-hunk merges, queue/`Order.md` semantic merges, delete-vs-modify where the other side already completed the same plan (accept the deletion), and comment/citation/formatting drift.
   ...
   **Escalate to explicit user sign-off** when either check cannot be cleared confidently: a summand collision whose independent-vs-duplicate status or correct value is unclear; a primary change that invalidates a session assumption and cannot be mechanically reconciled; overlapping edits to the same logic whose intents cannot both be preserved; or any reconcile you cannot affirmatively prove safe. **Default to escalation under uncertainty, not to proceeding.** Escalation waits on a human, so do it WITHOUT holding the PC-global landing lock: if it is already claimed (step 4), release it before the wait and re-claim + re-verify primary identity and tip after approval. Present a detailed summary derived from the verified final tree, not a generic completion message. Include:
   ...
   End with a direct confirmation question that makes clear no rebase or primary-branch update has occurred yet. Do not treat earlier plan approval, implementation approval, or general instructions to finish as sign-off for an escalation. Proceed only after an explicit affirmative response to this exact summary. If the user declines, requests changes, or does not answer, stop with both branches unchanged and the landing lock released. If the manifest, session tip, primary identity, primary tip, scope, residuals, or verification status changes after an escalation approval and before the rebase begins, invalidate the approval and present a refreshed summary for new confirmation.
...
7. For a session-worktree landing, rebase the session branch onto the current primary-branch `HEAD` and reconcile per step 3 — automatic by default, applying safety checks A (determinism-counter collision) and B (primary-delta vs session-assumption cross-check), escalating only on an unsafe/ambiguous conflict. Resolve conflicts without discarding either side. After every session-side reconciliation rebase, including a conflict-free one, rerun every affected review, build, runtime check, documentation update, and project-membership check, then regenerate the final-tree ledger and changed-file manifest for the rebased commit. Feed any failure back through root step 9's test -> fix -> retest loop; do not land until every affected testable item passes again. Commit only the reconciled session changes. If the reconcile (or a subsequent fix) triggered step 3's escalation conditions, release this session's landing lock before the human wait (do not hold the PC-global lock across it), present the refreshed step-3 escalation summary, and require renewed explicit confirmation before re-claiming the lock and advancing primary; if the user declines or does not answer, stop with primary unchanged and the lock released. A routine auto-reconcile that resolved cleanly and re-verified — including a determinism-summand collision that safety check A settled by the stacking rule — advances without sign-off. If the user cleaned primary by committing, that commit is the rebase base. If they used a stash, leave reapplication until after landing. Never merge primary into the session branch.
