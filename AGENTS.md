...
12. Land every repository mutation through this procedure. Steps 0–11 apply to C++ changes; step 12 also applies when the session changed only documentation, plans, skills, configuration, or other tracked files.
	1. Verify the fixed session-start baseline and changed-file list, then commit only reviewed session changes on the recorded session branch.
	2. Use installed `%LOCALAPPDATA%\BrokenEngine\AgentCli\v2\AgentCli.exe` (bootstrap through the `/compile` skill if missing or `--version` is not `2`). Generate an owner with `lock token`, resolve `git rev-parse --git-common-dir`, and claim `lock claim --domain landing --repo <git-common-dir> --owner <token> --session <label> --worktree <session-worktree>`. If held, report `lock status`; age is warning-only. Takeover requires explicit user approval and `lock steal` with the unchanged reported owner passed to `--expect`.
	3. Verify `lock status` reports this owner, the resolved primary checkout and target branch match the recorded identities, and the primary checkout is clean. Stop without releasing the claim if identity or cleanliness checks fail.
	4. Fetch no remote state. Read the current target-branch tip from the primary checkout and rebase the session branch onto it with `git rebase <target-branch>`. Resolve overlaps semantically through the normal rebase continue/abort flow; the later lander owns reconciliation. Never merge primary into the session branch, use `git rebase --rebase-merges`, or preserve a multi-parent commit.
	5. Re-run every review, selective/full build, runtime verification, documentation update, and project-membership check affected by the reconciliation. Commit only the reconciled session changes.
	6. Reverify ownership, primary cleanliness, and that primary is an ancestor of the session tip with no new merge commits. From the primary checkout run `git rebase <session-branch>`; because primary is already an ancestor, this only advances it to the verified session commit. Verify both checkouts resolve to the landed commit. If primary advanced, return to substep 4 and rebase/reverify.
	7. Reverify the landing owner and run `lock release --domain landing --repo <git-common-dir> --owner <token>`. Then owner-check and release any other session-scoped claim whose workflow requires landed verification, such as a plan claim, using that workflow's AgentCli domain, locator, and owner token. Never remove coordination state manually.
	8. Delete only this session's clean, landed worktree and branch after verifying their resolved identities and successful landing. Never force cleanup to bypass a failed check.
