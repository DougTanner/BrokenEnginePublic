...
1. Approve and classify. From user intent and an `implementer`'s repository preparation, main pins the objective, approved stage dispositions, tier and triggers, roles, acceptance checks, and execution card. Tier 1 and Tier 2 authorize implementation; Tier 3, Plan claim, reconciliation, and landing also require an execution card.
...
3. Implement and propagate. Main dispatches `implementer` slices, then an `implementer` for `/update-affected-code` after any C++ or GLSL change. Review-fix exceptions belong to `/resolve-findings`.
...
8. Reconcile, audit when triggered, and finalize. A final-evidence gate dispatches `/verify-changes` to a fresh read-only `reviewer`. Main dispatches one `/session-audit` reviewer only for late semantic fixes, manual conflict resolution or invalidated assumptions, Tier-3 cross-file integration, or contract-significant regions unseen by review. An `implementer` running `/finalize-changes` owns reconciliation, commit, landing, and claim release; WorktreeCli alone parses or mutates scheduler state. After all stage checks pass, wrapper sessions proceed to finalization by default, but main obtains the required confirmation before any primary mutation. Scheduler and landing mechanics: `.agents/skills/next-plan/references/execution-gates.md`. Landing completes only a repository stage; the session ends only when every stage is complete or explicitly deferred, with a tracked follow-up Plan where required.
