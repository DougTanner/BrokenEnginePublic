...
namespace toolcli
{
	namespace
	{
		...
		int RunReleaseAfterLanding(const Arguments& rArguments)
		{
			...
			if (!ReadClaim(claimPath, claim))
			{
				const DWORD targetAttributes = ::GetFileAttributesW(ExtendedLengthPath(claimPath).c_str());
				const DWORD targetError = targetAttributes == INVALID_FILE_ATTRIBUTES ? ::GetLastError() : ERROR_SUCCESS;
				if (targetAttributes != INVALID_FILE_ATTRIBUTES || (targetError != ERROR_FILE_NOT_FOUND && targetError != ERROR_PATH_NOT_FOUND))
				{
					return Conflict(L"release-after-landing", "target-claim-unproven", "terminal claim absence could not be proven");
				}
				std::error_code scanError;
				for (std::filesystem::directory_iterator it(ExtendedLengthPath(SchedulerRoot(repo) / L"claims"), scanError), end; !scanError && it != end; it.increment(scanError))
				{
					if (scanError)
					{
						return Conflict(L"release-after-landing", "competitor-scan-unproven", "terminal competitor scan could not be proven");
					}
					if (it->path().extension() != L".json")
					{
						continue;
					}
					if (!it->is_regular_file(scanError) || scanError)
					{
						return Conflict(L"release-after-landing", "competitor-scan-unproven", "terminal competitor scan could not be proven");
					}
					Claim competitor {};
					std::wstring competitorPlan;
					if (!ReadClaim(it->path(), competitor) || !competitor.json.contains("plan") || !competitor.json["plan"].is_string() || !NormalizePlanPath(Utf8ToWide(competitor.json["plan"].get<std::string>()), competitorPlan) || !ValidateClaim(competitor.json, repo, competitorPlan) || it->path().filename() != ClaimPath(SchedulerRoot(repo), competitorPlan).filename())
					{
						return Conflict(L"release-after-landing", "competitor-scan-unproven", "terminal competitor scan could not be proven");
					}
					const std::string state = competitor.json.value("state", "");
					if ((state == "preparing" || state == "awaiting-landing") && ClaimIsLive(competitor) && ClaimBaselineIsAncestorOfWorktreeHead(competitor, worktree)
						&& competitor.json["owner"] == receipt["owner"] && competitor.json["session"] == receipt["session"] && competitor.json["worktree"] == receipt["worktree"] && competitor.json["branch"] == receipt["branch"])
					{
						return Conflict(L"release-after-landing", "competing-terminal-claim", "another terminal claim prevents release recovery");
					}
				}
				if (scanError && scanError != std::errc::no_such_file_or_directory)
				{
					return Conflict(L"release-after-landing", "competitor-scan-unproven", "terminal competitor scan could not be proven");
				}
