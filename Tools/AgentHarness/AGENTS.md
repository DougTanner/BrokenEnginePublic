...
- Preserve the length-prefixed loopback JSON framing, request/response limits, the connect phase retried until the `--timeout-ms` deadline (not a single attempt), `--owner` heartbeat behavior, and exit codes (`0` success, `2` negative JSON response, `1` usage/transport/OS failure).
