	...
	for (int64_t i = 0; i < iMipCount; ++i, iFaceSize /= 2)
	{
		for (int64_t j = 0; j < 6; ++j)
		{
			...
			Pipeline pipeline(
			{
				...
				.ppShaders = {&gpShaderManager->mShaders.at(data::kShadersGltfFilterCubevertCrc), bIrradiance ? &gpShaderManager->mShaders.at(data::kShadersGltfIrradianceCubefragCrc) : &gpShaderManager->mShaders.at(data::kShadersGltfPrefilterEnvMapfragCrc)},
...
void TextureManager::GenerateGltfLutBrdf()
{
	...
	Pipeline pipeline(
	{
		...
		.ppShaders = {&gpShaderManager->mShaders.at(data::kShadersGltfGenBrdfLutvertCrc), &gpShaderManager->mShaders.at(data::kShadersGltfGenBrdfLutfragCrc)},
