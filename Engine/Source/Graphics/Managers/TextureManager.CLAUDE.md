...
Nine sampler types with runtime-configurable anisotropy; most share a global mip-LOD sharpen bias, but the water-normal sampler carries its own bias slider (zero-default) because the water shader's analytic-LOD variance lookup must track the hardware fetch LOD. The ctor chunk walk also copies each water normal map's header-baked per-mip Toksvig variance table (chunk headers are resident at startup; the lazy pixel data is not) for the per-frame uniform upload — see [Water/CLAUDE.md](../../../Data/Shaders/Water/CLAUDE.md) for the consuming technique. Three IBL cubemaps (irradiance, two pre-filtered radiance) loaded from pre-baked pack data via WaitForTextures().
