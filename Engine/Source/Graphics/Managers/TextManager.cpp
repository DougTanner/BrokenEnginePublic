...
TextManager::TextManager()
{
	...
	{
		Chunk& rChunk = rChunkMap.at(data::kFontsAunchantedXspaceBoldfntCrc);
		...
		LOG("Loading font {:#018x} with {} characters", data::kFontsAunchantedXspaceBoldfntCrc, iCharacters);
	...
	{
		Chunk& rChunk = rChunkMap.at(data::kFontsNotoSansSCLightfntCrc);
		int64_t iCharacters = rChunk.pHeader->fontHeader.iCharacters;
		auto pCharacterIds = reinterpret_cast<uint32_t*>(rChunk.pData);
		auto pCharacters = reinterpret_cast<common::Character*>(rChunk.pData + common::RoundUp(iCharacters * static_cast<int64_t>(sizeof(pCharacterIds[0])), common::kiAlignmentBytes));
		LOG("Loading font {:#018x} with {} characters", data::kFontsNotoSansSCLightfntCrc, iCharacters);
