...
- **LightingUniforms.cpp** - `RenderLightingGlobal` and `RenderLightingMain`: deposits (energy normalize, point light core radius), first spread (kernel world/size/sigma/decay, terrain cutoff), subsequent spread (kernel world/size/sigma/decay, pass count), combine (exposure, power, linear-clamp), directional/ambient weights, and water specular/skybox parameters
