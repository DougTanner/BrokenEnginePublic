...
`RenderFrameMain` drives the collection draw pipeline in fixed order: `BeginRender` (compute capacities, resize GPU buffers, reset counters) → per-coord `Render` (camera first for index-0 stability) → `EndRender` (write indirect-draw counts) → debug overlays. It also selects the camera's visible-area LOD and writes the water indirect-draw range plus the paired active-quad dims consumed in lockstep by the displacement compute pre-pass and the water vertex shader's `texelFetch`. Camera matrices (with Perlin camera shake folded into the view-projection), Gerstner wave params, and hex-shield uniforms are written last. When no active coord is renderable (every snapshot ring empty — a failed reconnect), the phase sequence is skipped, but the per-entity/effect + debug indirect counts are still flushed to zero: the record-once Main CB resubmits unconditionally, so any leftover nonzero count would ghost-draw phantom instances. That flush runs the `BeginRender`/`EndRender` pair (plus the debug pair) with no per-coord `Render` between, writing every collection counter as 0. The water LOD / displacement indirect params are deliberately left stale on that path — water is an always-draw fixed reference mesh (instanceCount = 1), so its self-consistent last-written per-framebuffer params keep the frozen-camera ocean rendering and need no flush (nor does lighting).
...
- Rotated octaves rotate cameraXY on the CPU by the same R(−θ) the shader uses, then fmod. For scrolling samples the per-frame reduced-time delta (a vec2) is instead rotated by R(γ = φ − θ), where φ is the sample's independent Speed Direction — the shader's R(θ) pattern rotation then cancels γ back to a world scroll direction R(φ)·(1,1), decoupled from the pattern rotation θ (φ = 0 gives the world (1,1) default). Rotating the reduced origin or time delta shader-side instead makes the wrap shift non-integer for any θ not a multiple of π/2, producing a visible pattern jump at every wrap; the shader consumes both outside its UV-rotation multiply. The delta magnitude stays √2 for all φ, so the fmod-10 wrap invariant holds for any scroll direction.
