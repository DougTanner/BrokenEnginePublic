...
- The lighting spread, combine, temporal, and history-copy chain shares one cadence epoch. A refresh atomically advances its current and history mappings and validity bounds, then publishes bounded work; a skipped epoch retains all of them and publishes zero chain work while deposit continues. Reset resolves from current data only, and history is refreshed only after temporal resolution.
