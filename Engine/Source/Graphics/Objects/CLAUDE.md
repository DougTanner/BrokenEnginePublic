...
- `ModelPipeline` snapshots the unmutated `PipelineInfo` at create (before the `kModel` flag is appended) so `Recreate()` — required after any auto-appended RTT texture is destroyed/recreated, since descriptor sets cache `VkImageView` at write time — can re-run the append path cleanly without tripping the empty-slot assert.
