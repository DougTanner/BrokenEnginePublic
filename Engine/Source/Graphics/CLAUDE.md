...
- `RenderMainPresentAcquire()`: Update camera, render scene, submit main command buffer, submit UI command buffer (ImGui), present to screen, acquire next image. Dispatched asynchronously via `mRenderFuture.Wake()` (`PersistentWorker` identified as `kThreadRender`)
