...
void CameraBase::CalculateMatricesAndVisibleArea()
{
	...
	if (iZoomBucket != miVisibleAreaZoomBucket || uiLatchKey != muiVisibleAreaLatchKey || iLod != miVisibleAreaLod)
	{
		...
		// LOD-stable shadow-region size: scale current visible-width-per-eye-distance ratio (constant
		// for a fixed FOV) to the LOD upper-bound eye distance. Within the LOD, mfLodStableShadow*
		// stays bit-stable across the per-integer-meter zoom-bucket re-latches that otherwise drive
		// shadow-texel-scale shimmer on Z motion. Re-latches only when LOD itself transitions (rare,
		// 5% hysteresis at 4^L boundaries). First-frame init via the zero-width guard.
		if (bLodChanged || mfLodStableShadowWidth == 0.0f)
		{
			...
			// visible width and shadows outside that region would sample clamp UVs. Inflate by the same
			...
			mfLodStableShadowWidth = (f4RenderVisibleArea.z - f4RenderVisibleArea.x) * fLodMaxEyeDistance / fSafeEyeDistance;
			mfLodStableShadowHeight = (f4RenderVisibleArea.y - f4RenderVisibleArea.w) * fLodMaxEyeDistance / fSafeEyeDistance;
		}
	}
