...
- **Allocation discipline**: Tracking is enabled only around the main loop — startup/teardown allocate freely. Suppression rules: root AGENTS.md; allocator/tracking mechanics: [Memory/AGENTS.md](Memory/AGENTS.md).
...
- `engine::LaunchOptions` (`LaunchOptions.h`, `gLaunchOptions`) parses `--agent-port` / `--log-file` / `--windowed WxH` at `wWinMain` and drives agent-channel activation, the optional log file sink, and an optional forced windowed client size. `ParseLaunchOptions` returns false and `wWinMain` aborts if `--agent-port` is outside `[1, 65535]` (htons would silently truncate it). Fullscreen resolves at the read sites (`Main.cpp`'s `WantedFullscreen()` helper) by precedence — a client-only agent override (set/cleared via `engine::SetAgentFullscreenOverride`, backing the harness `fullscreen` command) beats `--windowed`, which beats the persisted `gFullscreen` setting — and none of the three mutate `gFullscreen`.
- Agent-mode client launch (`--agent-port` set) shows the window with `SW_SHOWNOACTIVATE` and skips the `SetForegroundWindow`/`BringWindowToTop`/`SetFocus` trio, so a harness never steals foreground focus; it also boots with audio suspended (`gpAudioManager->Suspend()`), and the first `WM_SETFOCUS` (a human focusing the window) resumes it. Human launches are unchanged.
- Agent-mode clients also suppress all physical human input for the process lifetime: `engine::PhysicalInputSuppressed()` (`LaunchOptions.h`, true iff game `kbAgent` and `--agent-port` is set) gates the physical chokepoints in `WndProc`, `RawInputManager`, and `ImGuiManager::Prepare` so only the harness's synthetic `AgentInput` drives the client; the window-close / Alt+F4 escape hatch is never gated. Non-agent clients are unaffected — every gate sits behind the predicate. Input-path mechanics: [Input/AGENTS.md](Input/AGENTS.md).
