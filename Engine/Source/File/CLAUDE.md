...
**Threading**: Background `LoadingThread()` runs at background thread priority (`THREAD_MODE_BACKGROUND_BEGIN`) and processes queue sorted by priority, waking via condition variable. For non-texture chunks, sets state to `kDiskLoaded` and calls `NotifyChunkCompletion()` to wake waiters. Eager loading runs in separate async task that completes before background thread starts.
