...
public:
	...
	void Save(FileFlags_t fileFlags, const std::filesystem::path& rFilename, const SAVED_TYPE& rSavedEnd)
	{
		...
		LOG("Difference count: {} Last frame: {}", mHeader.iDifferenceCount, rSavedEnd.camera.iFrame);
...
public:
	...
	DifferenceStreamReader(const FileFlags_t& rFileFlags, const std::filesystem::path& rFilename, SAVED_TYPE& rSavedStart, DIFFERENCE_TYPE& rInitialDifference)
	{
		...
		LOG("Start/End: {} -> {} Difference count: {}", mHeader.savedStart.camera.iFrame, mHeader.savedEnd.camera.iFrame, mHeader.iDifferenceCount);
	...
	{
		if (mHeader.savedEnd.camera.iFrame == iFrame)
		{
			return false;
		}
