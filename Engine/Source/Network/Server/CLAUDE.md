...
- **Corrupt-packet isolation**: `Receive`'s dispatch switch is wrapped in try/catch — a corrupt count/size throws from the unchecked reader before any client state is mutated (handlers land parsed values in locals first), so the single packet is dropped at `kWarning` and the peer survives instead of being torn down (parity with `Client::Receive`).
- **Drain queues**: `Poll` clears these per poll: spawn requests, disconnects (client id + GUID so the game layer can persist fleet state), and opaque game packets. New subscriptions and resync client ids are *not* cleared in `Poll` — their consumers run only post-tick (`SendNewSubscriptionFullStates` / game `HandleResyncRequests`), which do not run while paused (`iFullTicks == 0`); each consumer clears its own queue after servicing, so a subscribe/resync accepted during a pause survives until the next tick instead of being dropped. Both persist-across-poll queues dedup on insert (a subscribe replaces any existing entry for the same `{client, slot}`; a resync skips an already-queued client id) so a burst while paused can't grow them unboundedly.
