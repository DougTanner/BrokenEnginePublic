...
- **ServerTypes** - Shared struct definitions: `ClientCoordSubscription`, `PendingSpawnRequest`, `PendingDisconnect` (carries the full list of owned `global_player_t` IDs and coords for that client), `PendingNewSubscription`, `GridUpdateData`, `PendingWeaponModeRequest` (carries a `global_player_t` identifying which player's weapon mode to toggle)
