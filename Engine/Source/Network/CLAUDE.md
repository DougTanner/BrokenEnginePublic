...
- **NetworkProtocol** - Wire protocol definitions: `PacketType` enum covering control packets (handshake, subscribe, ACK, debug, pause, timespeed, save/load, replay, reset) plus a `kGamePacketStart` sentinel — all packet type values at or above this sentinel are game-layer packets forwarded to the game session as raw bytes without engine interpretation. `ClientGuid` (128-bit persistent client identity wrapper) with `ClientGuidHash` for use in `unordered_map`, `ClientRequestFlags`, `AckState`, protocol constants, and LAN discovery constants
...
- **Client** (`gpClient`, `Client/`) - Client-side ENet peer with coord slot subscription state machine, per-slot ACK tracking, pipeline RTT/jitter/bandwidth measurement, and soft desync recovery. Packets with type values at or above `kGamePacketStart` are stored as raw bytes and drained by the game session for parsing
- **ClientSessionBase** (`Client/`, client-only) - Engine-generic base for game client sessions. Owns `Client` and `NetworkDiscoveryScanner`. Provides connection lifecycle, LAN discovery with auto-restart, coord subscription queue management, update buffering, and clock correction. The `CoordFrames::snapshots[]` ring is owned and driven by the game-layer reconciler
...
- **ServerTypes** - Shared pending-event structs (`PendingSpawnRequest`, `PendingDisconnect`, `PendingNewSubscription`, `GridUpdateData`, `ReceivedGamePacket`) used by `Server` and game layer. Game-specific pending structs live in the game-layer session
...
- **Server-authoritative save/load/reset**: Client sends `kClientSaveRequest`/`kClientLoadRequest`/`kClientResetRequest`; server performs the operation and broadcasts `kServerLoadNotification` after a load or reset. Each session calls `ResetClientsForLoad` (server) or `ResetForServerLoad` (client) to re-link player identities by GUID and clear buffered state. `kClientResetRequest` creates a fresh frame and resets the global ID counter to 1 before broadcasting
