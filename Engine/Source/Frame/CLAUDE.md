...
**Render** - Frame rendering orchestration populating GPU uniform buffers for shaders. `RenderFrameGlobal()` sets global uniforms (sun direction/color, shadows, terrain, water waves, wind simulation parameters) using individually named scalar fields on GlobalLayout. `RenderWindGlobal()` accumulates wind time using a static Timer and delta time scaled by the time scale wrapper, and writes all wind uniforms (propagation, time/global scaling, decay high/low, momentum high/low, threshold low/high/power, diffusion, smoke retention/power, texel size, wind time) to the global layout. Wind uses a ping-pong pattern with four pipelines (WindClear, WindClearTwo, WindSpread, WindSpreadTwo) targeting TextureOne and TextureTwo respectively. Each frame, `giWindTextureIndex` toggles (0↔1) to select which texture is the active write target; the active spread pipeline gets instance count 1 while the inactive gets 0, and `fWindTextureIndex` uniform tells smoke shaders which wind texture to sample. Wind deposit pipelines follow the same pattern via `kDynamicPipelineWindDeposit`/`kDynamicPipelineWindDepositTwo`. `RenderFrameMain()` sets per-frame uniforms (camera matrices, wave parameters, hex shields), resets skinning buffer allocations via `BufferManager::ResetSkinningAllocations()`, then delegates to `FrameInterpolate::Render()` for collection rendering. Includes day/night cycle helpers (DayPercent/NightPercent) and shared utilities for lighting and wind collections (`IsPointVisible`, `ProjectToBaseHeight`, `BuildAxisAlignedQuad`). `RenderObjects()` batch renders with visibility culling and transform setup. Sun angle is obtained from `Camera::SunAngle()` which handles UI override logic internally. Smoke simulation controlled via `gbSmokeClear`/`gbSmokeSpread` flags. Wind simulation controlled via `gbWindClear` flag.
