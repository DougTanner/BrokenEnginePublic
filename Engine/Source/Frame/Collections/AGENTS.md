...
- `Update` is mandatory and compile-checked. Other folded phase hooks are opt-in: declare a hook only when it has work, with the exact `ForEach*` signature; absent or non-invocable hooks skip silently. `static constexpr bool kbManualRender = true;` opts a collection out of the interpolate-render fold for an explicit manual render path.
