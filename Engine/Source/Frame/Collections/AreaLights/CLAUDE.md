...
- **Minimum deposit size** = 8 * max lighting-deposit texel size, preventing sub-texel flicker. The texel basis is the un-bumped detail texture size, not the lighting-headroom-bumped one — the headroom factor cancels in the shader's texel formula (inline comment in `Render` is authoritative).
...
## See Also
- [../CLAUDE.md](../CLAUDE.md) - `Collection<T>`, SOA, Sync pattern, GPU render phases, file-splitting conventions
