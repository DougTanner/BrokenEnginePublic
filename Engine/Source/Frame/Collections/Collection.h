...
// RENDER STATE BASE
// ============================================================================
// Base struct and helpers for render-only state that stays out of dual-buffered frame data.
...
// Grows capacity preserving existing data. Reuses GrowCapacityWithCopy.
template <typename TRenderState, typename TTuple>
void RenderStateEnsureCapacity(TRenderState& rState, int64_t iNeeded, TTuple&& members)
{
	if (iNeeded <= rState.iCapacity)
	{
		return;
	}

	GrowCapacityWithCopy(rState, iNeeded, rState.iCapacity, std::forward<TTuple>(members));
}

// Mirrors swap-with-last element removal. Takes iCount from the collection (not the render state).
template <typename TRenderState, typename TTuple>
void RenderStateSwapRemove(TRenderState& rState, int64_t iIndex, int64_t iCount, TTuple&& members)
{
	if (rState.pData != nullptr && iCount - 1 > iIndex)
	{
		std::apply([&](auto&... memberPtrRefs)
		{
			([&]()
			{
				if constexpr (std::is_array_v<std::remove_reference_t<decltype(memberPtrRefs)>>)
				{
					...
					for (size_t j = 0; j < N; ++j)
					{
						memberPtrRefs[j][iIndex] = memberPtrRefs[j][iCount - 1];
					}
				}
				else
				{
					memberPtrRefs[iIndex] = memberPtrRefs[iCount - 1];
				}
			}(), ...);
		}, std::forward<TTuple>(members));
	}
