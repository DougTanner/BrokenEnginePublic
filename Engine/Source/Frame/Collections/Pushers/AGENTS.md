...
- Carry-forward Update: Interpolate `Update` uses the full `Members()` copy fold at the existing Update phase — pushers have no self-driven simulation; last-synced state stays valid until the owner re-syncs. Do not move this copy into `AllocateAndCopy`.
