...
Client-only stationary wind-deposit splats — each radial deposits an outward impulse into the GPU wind field over a short keyframe lifetime. Spawned only by [Explosions](../Explosions/CLAUDE.md), which also registers the controller type (`Register()` here is an empty stub).
...
- Shares the A/B ping-pong wind-deposit indirect-count pattern with [WindTrails](../WindTrails/CLAUDE.md); on quad-buffer resize `BeginRender` rewrites the SSBO descriptor on both pipelines
