...
| **Explosions** | None | No | Composite effects spawning lights, puffs, trails, wind radials, and GPU particles. `ExplosionType` configures controller type indices for primary/secondary lights, puffs, trails, and wind; particle physics/color; secondary explosion count; and trail parameters. `SpawnInfo` provides per-instance scaling (light, size, smoke, time percentages), trail/particle counts and angles, color flags (kYellow, kRed), and self-destroy flag. Trails simulate gravity during Interpolate::Update. Fire-and-forget radial wind via `WindRadialsPostRender::AddControlled()`. GPU particle spawning is guarded by `FrameFlags::kRecalculated` to avoid duplicate particles during frame recalculation |
