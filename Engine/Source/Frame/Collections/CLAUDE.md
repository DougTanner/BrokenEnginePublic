...
- **Render-only state pattern**: SmokeTrails and WindTrails use file-scope statics keyed by UUID for previous-position tracking, keeping render state out of dual-buffered frame data. `EraseStaleRenderState<TMapType, TAccessor>()` (in `Collection.h`) prunes entries whose IDs are no longer present in any active frame, called from `BeginRender()` with a lambda that projects the collection's `idToIndexMap`
