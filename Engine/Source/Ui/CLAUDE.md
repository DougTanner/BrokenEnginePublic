...
- **CurveData / CurveWidget** (client-only) - Monotone cubic Hermite (Fritsch-Carlson) curve with a Photoshop-style ImGui editor. Capped at 16 control points; endpoints X-locked; interior points clamp between neighbors. Drag state is one shared function-local static — at most one widget instance may be interacted with at a time.
