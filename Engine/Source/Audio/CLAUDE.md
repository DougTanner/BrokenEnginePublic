...
- **StaticVoices** - 3D spatial one-shots + frame-driven looping/persistent voices. Hard-capped pool of XAudio2 source voices keyed by `crc` (retired voices are recycled, not destroyed). Recursive mutex (3D path re-enters 2D path). 3D sources asserted mono; the chunk-not-ready path requests a chunk load and bails for that frame.
