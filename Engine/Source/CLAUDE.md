...
- Agent-mode clients also suppress all physical human input for the process lifetime: `engine::PhysicalInputSuppressed()` (`LaunchOptions.h`, true iff game `kbAgent` and `--agent-port` is set) gates the physical chokepoints in `WndProc`, `RawInputManager`, and `ImGuiManager::Prepare` so only the harness's synthetic `AgentInput` drives the client; the window-close / Alt+F4 escape hatch is never gated. Non-agent clients are unaffected — every gate sits behind the predicate. Input-path mechanics: [Input/CLAUDE.md](Input/CLAUDE.md).
