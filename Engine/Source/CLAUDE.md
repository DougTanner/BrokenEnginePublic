...
### Main Loop Flow
**Client** (`BT_CLIENT`): Each frame processes Windows messages, handles fullscreen toggle, updates input (RawInputManager then game input conversion), runs fixed-rate physics steps with replay handling, renders current/interpolated frame, and updates audio. Fixed-rate physics at the game-defined rate (e.g., 64Hz) with variable-rate rendering and interpolated frames between physics ticks.
