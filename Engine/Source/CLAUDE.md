...
**Client/Server Split**: In client builds, `TickFrames()` calls `PollAndReconcileClient()` (clock correction and reconciliation wait/apply) before physics, early-returns when in desync debug mode (checked via `game::gpGame->GetDesyncTick() >= 0`), and calls `PostTickNetworkClient()` (network poll, reconcile kick, ACK send, flush) after physics. `Render()` (client-only) performs interpolated rendering and calls `PostRenderNetworkClient()` (post-render reconcile kick) at the end. `TickFramesAndRender()` (client-only, `#ifdef BT_CLIENT`) combines both into a single call. During extrapolation (connected to a server), the physics loop redirects `ActiveFrameRef` pointers to the per-coord snapshot ring buffer in `CoordFrames`: `PrepareExtrapolationTick()` advances the ring (reclaiming oldest pre-confirmed slots when full), `BuildExtrapolationFrameRef()` resolves per-coord references into the ring (called inside `BuildAndDispatchFrameTicks()`), and `RecordExtrapolationSnapshot()` captures CRCs after each tick (called inside `FinalizeFrameTick()`). The frame swap is skipped during extrapolation since the snapshot ring owns the frames. For rendering, `RenderFrame()` returns the latest snapshot frame for extrapolating coords, falling back to `mCoordFrames[coord].current`. In server builds, `TickFrames()` calls `PreTickNetworkServer()` before physics, then per-tick `PrepareServerTick()` recomputes the active set (so new client subscriptions from `FinalizeNewClientsServer` on the previous frame are picked up immediately) and ensures next frames. `FinalizeFrameTick()` calls `BroadcastServerTick()` which runs the server broadcast functions (FinalizeNewClientsServer, DetectPlayerDeathsServer, BroadcastStatusChangesServer, HandleSubscriptionUpdatesServer) followed by `NetworkServer::Flush()` to immediately dispatch all queued packets, ensuring clients receive updates for every simulated physics tick even when multiple physics steps run per tick.
...
### Main Loop Flow
**Client** (`BT_CLIENT`): Main.cpp's loop is minimal: processes Windows messages, handles fullscreen toggle, calls `ProcessInput()` (input), then `TickFrames()`, `Render()`, and audio update. Network orchestration is encapsulated inside GameBase: `TickFrames()` internally calls `PollAndReconcileClient()` before physics and `PostTickNetworkClient()` after physics; `Render()` internally calls `PostRenderNetworkClient()` after rendering. Desync debug mode is handled entirely within these GameBase methods via early returns. Audio runs unconditionally (including during desync debug mode). Fixed-rate physics at the game-defined rate (e.g., 64Hz) with variable-rate rendering and interpolated frames between physics ticks.

**Server** (`BT_SERVER`): Main.cpp's loop calls `TickFrames()` then `UpdateServerDisplayStats()`. Network orchestration is encapsulated inside GameBase: `TickFrames()` internally calls `PreTickNetworkServer()` (which polls the NetworkServer and NetworkDiscoveryResponder, handles disconnects, new clients, and spawn requests) before physics, then runs per-frame server broadcasts inside the physics loop. Sleeps between ticks via a high-resolution waitable timer to yield CPU. After physics, Main.cpp calls `UpdateServerDisplayStats()` to aggregate entity counts and memory stats, then repaints the GDI monitoring window every tick.
