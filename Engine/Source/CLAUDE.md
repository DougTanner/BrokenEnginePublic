...
**`SubscribedFrame`**: Defined in `GameBase.h`, the unified per-coord frame storage struct combining frame data with reconciliation state. Contains `current`/`next` dual-buffered `unique_ptr<Frame>` for all builds. In client builds (`#ifdef BT_CLIENT`), also contains: a fixed-size `snapshots` array of `SnapshotEntry` (each owning a `unique_ptr<Frame>`, frame number, CRC, input CRC) used during extrapolation; confirmed frame tracking as a snapshot index (`iConfirmedSnapshotIndex`) rather than a separate `unique_ptr`; per-coord `serverUpdates` map; an optional `PendingFullState` (owning a `unique_ptr<Frame>` with frame number); and a `uiGeneration` counter for stale-result detection. All per-coord state (frame data + reconciliation state) is stored in `mSubscribedFrames` (`unordered_map<GridCoord, SubscribedFrame>`).

**Architecture**: Owns the `mSubscribedFrames` map with swap-based dual-buffered updates. The update loop operates on active grid coordinates computed by the game: all five physics phases (Interpolate, PostRender, Collision, Transfer, Destroy/Spawn) are unified into `RunFrameTick()` and dispatched in parallel per-Frame via `Dispatch()` (controlled by `kbEnableFrameDispatch`; when disabled, frames run sequentially on the main thread for easier debugging). Pusher zone globals and Collision static members are `thread_local`, enabling safe parallel execution. Pre-resolves frame references into workbuffer-allocated `ActiveFrameRef` spans to avoid repeated map lookups across phases. Manages Frame ID assignment for per-Frame UUID generation without atomics, and owns the authoritative frame counter (exposed via `FrameCounter()`) and simulation time. Provides grid serialization for save/load (deterministic coordinate ordering) and a replay system using DifferenceStream for delta-compressed deterministic recording/playback with CRC validation. Replay loading reads metadata (human grid coordinate, player ID, armor) from a separate `.replay.meta` file, restricts the active set to only the human's grid coordinate, skips multi-frame transfers/EnsureNextFrames, and injects pending transfer StatusChanges into FrameInput between `LoadDifference()` and `ValidateChecksum()` for deterministic cross-cell replay. `HarvestTransfers()` is called only in server builds (`#ifdef BT_SERVER`); the client receives cross-coord transfers as StatusChanges from the server rather than locally harvesting them.

**Client/Server Split**: `UpdateFramesAndRender()` (client-only, `#ifdef BT_CLIENT`) runs the full physics-plus-rendering loop with interpolated frames and GPU rendering. During extrapolation (connected to a server), the physics loop redirects `ActiveFrameRef` pointers to the per-coord snapshot stack in `SubscribedFrame`: `PrepareExtrapolationTick()` advances the stack, `BuildExtrapolationFrameRef()` resolves per-coord references into the stack, and `RecordExtrapolationSnapshot()` captures CRCs after each tick. The frame swap is skipped during extrapolation since the snapshot stack owns the frames. For rendering, `RenderFrame()` returns the latest snapshot frame for extrapolating coords, falling back to `mSubscribedFrames[coord].current`. `UpdateFrames()` runs physics updates only, suitable for headless server builds. Both share the same fixed-timestep update logic and frame management. In server builds, `UpdateFrames()` calls the server broadcast functions (FinalizeNewClientsServer, DetectPlayerDeathsServer, BroadcastStatusChangesServer, HandleSubscriptionUpdatesServer) inside the physics loop after each frame's updates, followed by `NetworkServer::Flush()` to immediately dispatch all queued packets, ensuring clients receive updates for every simulated physics frame even when multiple physics steps run per tick.
...
### Main Loop Flow
**Client** (`BT_CLIENT`): Each frame processes Windows messages (passes `bIgnoreFocus=true` to avoid blocking on `GetMessage()` when alt-tabbed), handles fullscreen toggle, updates input (RawInputManager then game input conversion), then checks for desync debug mode: if `GetDesyncFrame() >= 0`, the loop only polls the network client, flushes, and renders (skipping reconciliation, physics, and audio) to keep the window responsive while awaiting the server's debug frame response. Otherwise, the normal path waits for and applies the async reconciliation result via `WaitForReconcile()` with soft clock correction computed first (proportionally adjusts `mUpdateRemainderNs` to keep the client behind the server by RTT/2 + 1 frames, enabling the CRC fast-path to succeed routinely), then frame deficit compensation (adds the number of frames rewound by reconciliation, reduced by the clock error so rollbacks help correction converge, to `mUpdateRemainderNs` so the subsequent physics loop does not re-advance past the reconciled position), runs fixed-rate physics steps with replay handling via `UpdateFrames()`, polls the network client via `PollNetworkClient()` and kicks the next reconciliation via `TryKickReconcile()` (between physics and render, so the async worker gets the freshest network data and latest extrapolated snapshots), sends ACK state via `NetworkClient::SendAck()` and flushes the network client (`NetworkClient::Flush()`) to immediately dispatch queued packets (after physics and poll/kick, so the flush carries the most up-to-date ACK state), renders current/interpolated frame via `Render()`, then updates audio. `UpdateFramesAndRender()` still exists as a convenience that calls `UpdateFrames()` then `Render()`, but Main.cpp calls them separately to insert poll+kick between physics and render. Two dedicated CPU timers (`kCpuTimerNetworkPollReconcile` and `kCpuTimerNetworkSend`) bracket the network phases. Fixed-rate physics at the game-defined rate (e.g., 64Hz) with variable-rate rendering and interpolated frames between physics ticks.
...
### Memory Patterns
- Frame state uses `SubscribedFrame` structs in `mSubscribedFrames` map with dual-buffered `current`/`next` frames per grid coordinate for deterministic updates
