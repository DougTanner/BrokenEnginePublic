...
- Skinning uses compact three-row joint matrices in a buffer separate from mesh data. The skinned normal path combines the mesh normal matrix with `mat3(skinMatrix)` and therefore assumes near-rigid joint transforms rather than arbitrary non-uniform joint scaling.
