...
- **ShaderFunctions.h** - Common GLSL utilities for coordinate transforms, lighting (`DirectionalLighting` normal-weights EWNS channels and applies hue-preserving pow via a luminance/average scalar blend controlled by `fPowerMode`; `WaterLighting` does the same with softened normals and a three-term power curve; `AmbientLighting` averages all EWNS directions with the same hue-preserving pow; `Lighting` samples directional and ambient EWNS textures, sums them, and applies normal weighting to the combined result), specular highlights (`Specular`), sun and moon lighting (`SunLighting` applies Cook-Torrance BRDF for both sun and moon directional lights), normal mapping, parallax projection (`BaseHeightPosition`), smoke functions (`WorldToSmokeTexcoord`, `SmokeShadow`, `AddSmoke`, `BlendSmoke` — all use `fSmokeLightingMultiplier` from the global layout uniform to scale directional lighting contribution, including moon), shadow stretch projection (`ShadowStretchProjection`), and a deposit edge fade utility that fades deposit output to zero near the lighting texture boundary (prevents popping as deposits move in or out of the lighting area)
...
- **Four-channel directional lighting**: RGB stored as separate render targets with EWNS directional weights; `Lighting` sums directional and ambient EWNS samples together, then passes the combined result through the normal-weighted path — both contributions are normal-weighted
- **World-space directional deposit**: Deposit shaders receive interpolated world position and a center point from the vertex shader. Both area lights and point lights compute EWNS direction weights from the fragment's world-space offset to the light's center point, using cos^2 lobe weighting with an epsilon fallback to omnidirectional near the light center
