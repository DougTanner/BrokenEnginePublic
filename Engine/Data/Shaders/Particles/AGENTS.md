...
- **Billboards.vert/.frag** - Separate screen-space system that shares the directory but not the particle pipeline: no spawn/update, no allocation bitmap, quads placed directly in NDC with aspect correction. Consumed by the engine-frame client-only Billboards collection via a dynamic pipeline with alpha blend and a host-visible indirect count — not interchangeable with the additive particle path

## Architecture Notes

- Indirect everything: spawn writing the update dispatch and render draw commands removes any CPU knowledge of live counts — zero readback, zero fences. The spawn dispatch ceil-divide references `kiParticleUpdateGroupSize` (the same constant `local_size_x` in update uses), so both shaders stay in sync on a group-size change
...
- Update and render shaders read SSBO struct fields one at a time; spawn's slot fill is a whole-struct copy. The field-by-field style is not a workaround — an earlier claim that glslang drops trailing fields on scalar-block-layout SSBO struct copies was investigated (no upstream glslang issue; SPIR-V disassembly at VulkanSDK 1.4.341.1 emits complete whole-struct copies for both the spawn store and the Model read-into-local sites) and not substantiated; either style is valid
