...
- Indirect everything: spawn writing the update dispatch and render draw commands removes any CPU knowledge of live counts — zero readback, zero fences. The spawn dispatch math hardcodes `(count + 31) / 32` with a comment instead of referencing `kiParticleUpdateGroupSize` (which update's `local_size_x` does reference), so changing the group size means fixing spawn's math by hand
