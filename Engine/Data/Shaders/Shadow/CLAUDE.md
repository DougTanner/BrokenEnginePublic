...
- **Rate-limited texel scale + centered visible window**: the terrain shadow texel world size is set CPU-side from the camera's rate-limited shadow-texel height (fixed at a settled eye height, so the grid is snap-stable under pan; rescaled only on the slow crawl while tracking a zoom). The shaders read it through the area uniforms and are otherwise agnostic to whether it moves. Shadow.comp and the terrain blur passes early-out outside a centered visible sub-window (the `iShadowVisible*` uniforms) expanded by a `kiShadowWindowMargin` border; only that window is ray-marched/blurred each frame. The window holds a constant on-screen pixel size at every settled height (the texels coarsen with zoom instead of cropping coverage); only a fast zoom-out that outruns the ramp transiently grows it toward the full texture, and an overflowing sample reads the CLAMP_TO_BORDER white edge (`mVkSamplerBorderWhite`) — opaque white = no-shadow, since the shadow texture is the inverse coverage. Out-of-window texels write the unshadowed lit value so blur edge reads stay consistent; the margin guarantees the consumer's bilinear edge reads land on current-frame, fully-blurred texels
