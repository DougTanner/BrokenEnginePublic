...
- **Sun/moon tint on dominant lighting**: both the directional `WaterLighting` result (`f3WaterLightingMults`) and the additive `AmbientLightingPrecomputed` result (`f3WaterNewAmbient`) are multiplied by `mix(vec3(1), f4SunMoonColor.xyz, fLightingWaterMoonTint)`. This makes `gMoonBrightness` (a per-channel floor on `f4SunMoonColor`) drive water at night the same way `SunLighting()` drives terrain — preserving the moon's blue-shifted hue rather than collapsing to a scalar average. Default tint = 1.0 mirrors terrain's moon response; lower values soften the night-time darkening.
