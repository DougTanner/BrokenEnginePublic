...
- **Precision-safe normal sampling**: `textureGrad` with derivatives of the camera-relative position and `fract()`-wrapped UVs — derivatives are taken before per-octave scaling so mip selection stays stable across summed octaves. Normal source is BC5; Z reconstructed in-shader as `sqrt(saturate(1 - x*x - y*y))`.
