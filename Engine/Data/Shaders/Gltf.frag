...
layout (binding = 11) buffer gltfMaterialsUniform
{
...
#if 0 // DT:
	vec3 tangentNormal = texture(normalMap, material.normalTextureSet == 0 ? f2InUV0 : f2InUV1).xyz * 2.0 - 1.0;
#else
...
void main()
{
	...
	if (material.alphaMask == 1.0f) {
		...
		#if 0 // DT:
			baseColor = SRGBtoLINEAR(texture(colorMap, material.baseColorTextureSet == 0 ? f2InUV0 : f2InUV1)) * material.baseColorFactor;
		#else
			baseColor = SRGBtoLINEAR(texture(colorMap, f2InUV)) * material.baseColorFactor;
		#endif
		} else {
	...
	if (material.workflow == PBR_WORKFLOW_METALLIC_ROUGHNESS) {
		...
		#if 0 // DT:
			vec4 mrSample = texture(physicalDescriptorMap, material.physicalDescriptorTextureSet == 0 ? f2InUV0 : f2InUV1);
		#else
	...
	if (material.workflow == PBR_WORKFLOW_SPECULAR_GLOSINESS) {
		...
		if (material.physicalDescriptorTextureSet > -1) {
		#if 0 // DT:
			perceptualRoughness = 1.0 - texture(physicalDescriptorMap, material.physicalDescriptorTextureSet == 0 ? f2InUV0 : f2InUV1).a;
		#else
	...
	vec3 n = (material.normalTextureSet > -1) ? getNormal() : f3InNormal;
	...
	#if 0 // DT:
		float ao = texture(aoMap, (material.occlusionTextureSet == 0 ? f2InUV0 : f2InUV1)).r;
	#else
	...
	#if 0 // DT:
		vec3 emissive = SRGBtoLINEAR(texture(emissiveMap, material.emissiveTextureSet == 0 ? f2InUV0 : f2InUV1)).rgb * u_EmissiveFactor;
	#else
	...
	if (mainLayout.fGltfDebugViewInputs > 0.0) {
		...
		#if 0 // DT:
			case 1:
				f4OutColor.rgba = material.baseColorTextureSet > -1 ? texture(colorMap, material.baseColorTextureSet == 0 ? f2InUV0 : f2InUV1) : vec4(1.0f);
				break;
			case 2:
				f4OutColor.rgb = (material.normalTextureSet > -1) ? texture(normalMap, material.normalTextureSet == 0 ? f2InUV0 : f2InUV1).rgb : normalize(f3InNormal);
				break;
			case 3:
				f4OutColor.rgb = (material.occlusionTextureSet > -1) ? texture(aoMap, material.occlusionTextureSet == 0 ? f2InUV0 : f2InUV1).rrr : vec3(0.0f);
				break;
			case 4:
				f4OutColor.rgb = (material.emissiveTextureSet > -1) ? texture(emissiveMap, material.emissiveTextureSet == 0 ? f2InUV0 : f2InUV1).rgb : vec3(0.0f);
				break;
		#else
			case 1:
				f4OutColor.rgba = material.baseColorTextureSet > -1 ? texture(colorMap, f2InUV) : vec4(1.0f);
				break;
			case 2:
				f4OutColor.rgb = (material.normalTextureSet > -1) ? texture(normalMap, f2InUV).rgb : normalize(f3InNormal);
				break;
			case 3:
				f4OutColor.rgb = (material.occlusionTextureSet > -1) ? texture(aoMap, f2InUV).rrr : vec3(0.0f);
				break;
			case 4:
				f4OutColor.rgb = (material.emissiveTextureSet > -1) ? texture(emissiveMap, f2InUV).rgb : vec3(0.0f);
				break;
		#endif
			case 5:
