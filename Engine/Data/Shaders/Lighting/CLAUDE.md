...
- **AreaLight.frag** - Renders oriented area lights into MRT color attachments with EWNS directional weighting. Computes direction from the world center point, using the same approach as point lights
...
- **LightingSpread.frag** - Radial directional spread fragment shader with MRT output; samples all three color channels simultaneously across a configurable number of rings, each with a configurable base direction count (increasing by one per ring) and per-ring angular jitter. Applies EWNS directional weighting, distance-based weight falloff, and a decay multiplier. Samples the terrain elevation texture (set 1, binding 4) for two independent height-based attenuation factors: spread distance is reduced using a factor normalized over the 0–islandHeight range, while decay (intensity) is reduced using a factor normalized over the baseHeight–targetHeight range (only above base height). Runs as N passes (configurable via a runtime slider), each reading the previous pass's output and writing to its own set of spread textures via a dedicated spread render pass
- **LightCombine.comp** - Tone maps accumulated float16 lighting to UNORM8 output for all three color channels in a single dispatch. Applies Uchimura tone curve (piecewise toe/linear/shoulder HDR-to-LDR mapping with six artist-controllable parameters) in two modes controlled by `fCombineHuePreserve` (0–1 blend): per-channel mode (0) applies the tone curve independently to R/G/B, while hue-preserving mode (1) computes per-direction luminance across R/G/B channels and tone-maps the scalar luminance, then scales all channels uniformly to preserve hue. Reads sampler arrays (one entry per spread pass per color channel) and sums all spread pass outputs before tone mapping
...
**Area vs. point directional deposit**: Both `AreaLight.frag` and `PointLight.frag` compute EWNS direction weights from the fragment's world-space offset to the light's center point. Both use cos^2 lobe EWNS weighting with an epsilon fallback to omnidirectional when the fragment is very close to the light center.
