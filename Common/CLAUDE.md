...
- **Workbuffer** - Push/pop stack allocator for reusable scratch memory, eliminating per-frame heap allocations. Accessed via `gpThreadLocal->mWorkbuffer`. Supports raw buffers, string building, and typed element operations with nested usage. `ScopedWorkbufferPop` is a RAII wrapper that pops the workbuffer on destruction, holds a `const char*` result, and provides an implicit conversion to `const char*` (also has a `std::formatter` specialization for direct use in `std::format`/`Log()`). `ShrinkLastPushBuffer()` trims the most recent push buffer to its current content length
