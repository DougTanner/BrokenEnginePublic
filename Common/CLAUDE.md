...
- **Multithreading** - Worker pool accessed via `common::gpMultithreading`. `Dispatch(count, processRange)` splits work across all workers plus the main thread, then joins; propagates `miLogTickCounter` and `miLogIndent` from the calling thread to workers so log output remains correctly tagged and indented. `IsMainThread()` guards operations that must not run on worker threads. The 3-arg constructor creates a non-global instance for dedicated dispatch pools
...
- **ThreadLocal** - Per-thread storage owning a log buffer, `Workbuffer`, optional `miLogTickCounter` for tick-prefixed log output, and `miLogIndent` for hierarchical log indentation. Configures deterministic floating-point math on every thread. Accessed via `common::gpThreadLocal`
